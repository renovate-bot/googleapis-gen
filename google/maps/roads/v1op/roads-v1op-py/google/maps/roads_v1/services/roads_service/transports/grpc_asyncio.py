...
class RoadsServiceGrpcAsyncIOTransport(RoadsServiceTransport):
    """gRPC AsyncIO backend transport for RoadsService.
    ...
    def snap_to_roads(self) -> Callable[
        ...
        Returns:
            Callable[[~.SnapToRoadsRequest],
                    Awaitable[~.SnapToRoadsResponse]]:
        ...
        if 'snap_to_roads' not in self._stubs:
            self._stubs['snap_to_roads'] = self.grpc_channel.unary_unary(
                '/google.maps.roads.v1op.RoadsService/SnapToRoads',
                request_serializer=roads.SnapToRoadsRequest.serialize,
                response_deserializer=roads.SnapToRoadsResponse.deserialize,
            )
        return self._stubs['snap_to_roads']
    ...
    def list_nearest_roads(self) -> Callable[
        ...
        Returns:
            Callable[[~.ListNearestRoadsRequest],
                    Awaitable[~.ListNearestRoadsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'list_nearest_roads' not in self._stubs:
            self._stubs['list_nearest_roads'] = self.grpc_channel.unary_unary(
                '/google.maps.roads.v1op.RoadsService/ListNearestRoads',
                request_serializer=roads.ListNearestRoadsRequest.serialize,
                response_deserializer=roads.ListNearestRoadsResponse.deserialize,
            )
