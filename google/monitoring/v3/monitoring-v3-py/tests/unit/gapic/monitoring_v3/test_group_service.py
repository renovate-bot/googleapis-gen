...
from google.cloud.monitoring_v3.services.group_service.transports.base import _API_CORE_VERSION
from google.cloud.monitoring_v3.services.group_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test__get_default_mtls_endpoint():
    ...
    assert GroupServiceClient._get_default_mtls_endpoint(None) is None
    assert GroupServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert GroupServiceClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert GroupServiceClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert GroupServiceClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test_group_service_client_get_transport_class():
    ...
    transport = GroupServiceClient.get_transport_class("grpc")
    assert transport == transports.GroupServiceGrpcTransport


@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (GroupServiceClient, transports.GroupServiceGrpcTransport, "grpc"),
    (GroupServiceAsyncClient, transports.GroupServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
@mock.patch.object(GroupServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GroupServiceClient))
@mock.patch.object(GroupServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GroupServiceAsyncClient))
def test_group_service_client_client_options(client_class, transport_class, transport_name):
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (GroupServiceClient, transports.GroupServiceGrpcTransport, "grpc", "true"),
    (GroupServiceAsyncClient, transports.GroupServiceGrpcAsyncIOTransport, "grpc_asyncio", "true"),
    (GroupServiceClient, transports.GroupServiceGrpcTransport, "grpc", "false"),
    (GroupServiceAsyncClient, transports.GroupServiceGrpcAsyncIOTransport, "grpc_asyncio", "false"),
])
@mock.patch.object(GroupServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GroupServiceClient))
@mock.patch.object(GroupServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GroupServiceAsyncClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_group_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_group_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_group_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
async def test_list_groups_async(transport: str = 'grpc_asyncio', request_type=group_service.ListGroupsRequest):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(group_service.ListGroupsResponse(
            next_page_token='next_page_token_value',
        ))
...
def test_list_group_members_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in pager]
        assert len(results) == 6
        assert all(isinstance(i, monitored_resource.MonitoredResource)
                   for i in results)
...
def test_group_service_base_transport():
    # Instantiate the base transport.
    with mock.patch('google.cloud.monitoring_v3.services.group_service.transports.GroupServiceTransport.__init__') as Transport:
...
def test_group_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.monitoring_v3.services.group_service.transports.GroupServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_group_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.monitoring_v3.services.group_service.transports.GroupServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.GroupServiceTransport()
        adc.assert_called_once()
...
def test_group_service_transport_create_channel_old_api_core(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
...
@requires_api_core_lt_1_26_0
def test_group_service_transport_create_channel_user_scopes(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
...
@pytest.mark.parametrize("transport_class", [transports.GroupServiceGrpcTransport, transports.GroupServiceGrpcAsyncIOTransport])
def test_group_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.GroupServiceGrpcTransport, transports.GroupServiceGrpcAsyncIOTransport])
def test_group_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.GroupServiceGrpcTransport, transports.GroupServiceGrpcAsyncIOTransport])
def test_group_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_parse_group_path():
    ...
    path = GroupServiceClient.group_path(**expected)

    # Check that the path construction is reversible.
    actual = GroupServiceClient.parse_group_path(path)
    assert expected == actual

def test_common_billing_account_path():
    billing_account = "oyster"
    expected = "billingAccounts/{billing_account}".format(billing_account=billing_account, )
    actual = GroupServiceClient.common_billing_account_path(billing_account)
    assert expected == actual
...
def test_parse_common_location_path():
    ...
    path = GroupServiceClient.common_location_path(**expected)

    # Check that the path construction is reversible.
    actual = GroupServiceClient.parse_common_location_path(path)
    assert expected == actual
