...
from google.cloud.appengine_admin_v1.services.domain_mappings import DomainMappingsAsyncClient
...
def test_domain_mappings_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_list_domain_mappings(transport: str = 'grpc', request_type=appengine.ListDomainMappingsRequest):
    client = DomainMappingsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_domain_mappings_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, domain_mapping.DomainMapping)
                   for i in results)
...
def test_create_domain_mapping(transport: str = 'grpc', request_type=appengine.CreateDomainMappingRequest):
    client = DomainMappingsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_domain_mapping(transport: str = 'grpc', request_type=appengine.UpdateDomainMappingRequest):
    client = DomainMappingsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_delete_domain_mapping(transport: str = 'grpc', request_type=appengine.DeleteDomainMappingRequest):
    client = DomainMappingsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.DomainMappingsGrpcTransport, transports.DomainMappingsGrpcAsyncIOTransport])
def test_domain_mappings_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
