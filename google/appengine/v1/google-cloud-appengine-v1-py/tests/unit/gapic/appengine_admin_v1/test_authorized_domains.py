...
from google.cloud.appengine_admin_v1.services.authorized_domains import AuthorizedDomainsAsyncClient
...
def test__get_default_mtls_endpoint():
    ...
    assert AuthorizedDomainsClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert AuthorizedDomainsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert AuthorizedDomainsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert AuthorizedDomainsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert AuthorizedDomainsClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (AuthorizedDomainsAsyncClient, transports.AuthorizedDomainsGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_authorized_domains_client_client_options(client_class, transport_class, transport_name):
...
def test_authorized_domains_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_authorized_domains_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_authorized_domains_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_authorized_domains_client_client_options_from_dict():
    with mock.patch('google.cloud.appengine_admin_v1.services.authorized_domains.transports.AuthorizedDomainsGrpcTransport.__init__') as grpc_transport:
...
def test_list_authorized_domains(transport: str = 'grpc', request_type=appengine.ListAuthorizedDomainsRequest):
    client = AuthorizedDomainsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_authorized_domains_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, domain.AuthorizedDomain)
                   for i in results)
...
def test_authorized_domains_base_transport():
    ...
    with mock.patch('google.cloud.appengine_admin_v1.services.authorized_domains.transports.AuthorizedDomainsTransport.__init__') as Transport:
...
@pytest.mark.parametrize("transport_class", [transports.AuthorizedDomainsGrpcTransport, transports.AuthorizedDomainsGrpcAsyncIOTransport])
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.AuthorizedDomainsGrpcTransport, transports.AuthorizedDomainsGrpcAsyncIOTransport])
def test_authorized_domains_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_client_withDEFAULT_CLIENT_INFO():
    ...
    with mock.patch.object(transports.AuthorizedDomainsTransport, '_prep_wrapped_messages') as prep:
        client = AuthorizedDomainsClient(
            credentials=credentials.AnonymousCredentials(),
