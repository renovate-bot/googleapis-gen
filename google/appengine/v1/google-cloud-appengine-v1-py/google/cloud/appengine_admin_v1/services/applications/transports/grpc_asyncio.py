...
class ApplicationsGrpcAsyncIOTransport(ApplicationsTransport):
    """gRPC AsyncIO backend transport for Applications.
    ...
    def get_application(self) -> Callable[
        ...
        if 'get_application' not in self._stubs:
            self._stubs['get_application'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=appengine.GetApplicationRequest.serialize,
    ...
    def repair_application(self) -> Callable[
        ...
        Returns:
            Callable[[~.RepairApplicationRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'repair_application' not in self._stubs:
