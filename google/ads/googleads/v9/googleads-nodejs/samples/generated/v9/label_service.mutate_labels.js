...
function main(customerId, operations) {
  // [START googleads_v9_generated_LabelService_MutateLabels_async]
  /**
  ...
  const {LabelServiceClient} = require('google-ads').v9;
  ...
  async function mutateLabels() {
    ...
    // Run request
    const response = await googleadsClient.mutateLabels(request);
    console.log(response);
  }

  mutateLabels();
  // [END googleads_v9_generated_LabelService_MutateLabels_async]
}
