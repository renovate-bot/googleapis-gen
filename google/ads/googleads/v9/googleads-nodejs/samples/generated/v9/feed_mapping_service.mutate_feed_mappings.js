...
function main(customerId, operations) {
  // [START googleads_v9_generated_FeedMappingService_MutateFeedMappings_async]
  /**
  ...
  const {FeedMappingServiceClient} = require('google-ads').v9;

  // Instantiates a client
  const googleadsClient = new FeedMappingServiceClient();

  async function mutateFeedMappings() {
    // Construct request
    const request = {
      customerId,
      operations,
    ...
    // Run request
    const response = await googleadsClient.mutateFeedMappings(request);
    console.log(response);
  }

  mutateFeedMappings();
  // [END googleads_v9_generated_FeedMappingService_MutateFeedMappings_async]
}
