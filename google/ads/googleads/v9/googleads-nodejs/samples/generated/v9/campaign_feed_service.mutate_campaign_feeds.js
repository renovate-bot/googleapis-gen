...
function main(customerId, operations) {
  // [START googleads_v9_generated_CampaignFeedService_MutateCampaignFeeds_async]
  /**
  ...
  const {CampaignFeedServiceClient} = require('google-ads').v9;

  // Instantiates a client
  const googleadsClient = new CampaignFeedServiceClient();

  async function mutateCampaignFeeds() {
    // Construct request
    const request = {
      customerId,
      operations,
    ...
    // Run request
    const response = await googleadsClient.mutateCampaignFeeds(request);
    console.log(response);
  }

  mutateCampaignFeeds();
  // [END googleads_v9_generated_CampaignFeedService_MutateCampaignFeeds_async]
}
