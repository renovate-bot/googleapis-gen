...
function main(customerId, operations) {
  // [START googleads_v9_generated_CampaignDraftService_MutateCampaignDrafts_async]
  /**
  ...
  const {CampaignDraftServiceClient} = require('google-ads').v9;

  // Instantiates a client
  const googleadsClient = new CampaignDraftServiceClient();

  async function mutateCampaignDrafts() {
    // Construct request
    const request = {
      customerId,
      operations,
    ...
    // Run request
    const response = await googleadsClient.mutateCampaignDrafts(request);
    console.log(response);
  }

  mutateCampaignDrafts();
  // [END googleads_v9_generated_CampaignDraftService_MutateCampaignDrafts_async]
}
