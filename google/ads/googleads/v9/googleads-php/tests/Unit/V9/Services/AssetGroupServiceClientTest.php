...
use Google\Ads\GoogleAds\V9\Services\MutateAssetGroupsResponse;
...
class AssetGroupServiceClientTest extends GeneratedTest
{
     ...
     * @test
     */
    public function getAssetGroupTest()
    {
        ...
        $name = 'name3373707';
        $path1 = 'path1106438828';
        $path2 = 'path2106438829';
        $expectedResponse = new AssetGroup();
        $expectedResponse->setResourceName($resourceName2);
        $expectedResponse->setId($id);
        $expectedResponse->setCampaign($campaign);
        $expectedResponse->setName($name);
        $expectedResponse->setPath1($path1);
        $expectedResponse->setPath2($path2);
        $transport->addResponse($expectedResponse);
        // Mock request
        $formattedResourceName = $client->assetGroupName('[CUSTOMER_ID]', '[ASSET_GROUP_ID]');
        $response = $client->getAssetGroup($formattedResourceName);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v9.services.AssetGroupService/GetAssetGroup', $actualFuncCall);
     ...
     * @test
     */
    public function getAssetGroupExceptionTest()
    {
        $transport = $this->createTransport();
        $client = $this->createClient([
            'transport' => $transport,
        ...
        $formattedResourceName = $client->assetGroupName('[CUSTOMER_ID]', '[ASSET_GROUP_ID]');
        try {
            $client->getAssetGroup($formattedResourceName);
    ...
    public function mutateAssetGroupsTest()
    {
        ...
        $operations = [];
        $response = $client->mutateAssetGroups($customerId, $operations);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
        $actualRequestObject = $actualRequests[0]->getRequestObject();
        $this->assertSame('/google.ads.googleads.v9.services.AssetGroupService/MutateAssetGroups', $actualFuncCall);
