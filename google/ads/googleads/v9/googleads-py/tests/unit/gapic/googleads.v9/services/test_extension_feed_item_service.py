...
def test_extension_feed_item_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v9.services.services.extension_feed_item_service.ExtensionFeedItemServiceClient.get_transport_class') as gtc:
...
def test_extension_feed_item_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v9.services.services.extension_feed_item_service.transports.ExtensionFeedItemServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ExtensionFeedItemServiceTransport()
        adc.assert_called_once()
