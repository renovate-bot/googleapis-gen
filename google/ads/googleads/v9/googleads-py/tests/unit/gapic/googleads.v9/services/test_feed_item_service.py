...
def test_feed_item_service_client_client_options():
    # Check that if channel is provided we won't create a new one.
    with mock.patch('google.ads.googleads.v9.services.services.feed_item_service.FeedItemServiceClient.get_transport_class') as gtc:
    ...
    with mock.patch('google.ads.googleads.v9.services.services.feed_item_service.FeedItemServiceClient.get_transport_class') as gtc:
        client = FeedItemServiceClient(transport="grpc")
        gtc.assert_called()
    ...
    with mock.patch('google.ads.googleads.v9.services.services.feed_item_service.transports.FeedItemServiceGrpcTransport.__init__') as grpc_transport:
        ...
        client = FeedItemServiceClient(client_options=options)
...
def test_feed_item_service_client_mtls_env_auto(use_client_cert_env):
...
def test_get_feed_item_from_dict():
    test_get_feed_item(request_type=dict)
...
def test_mutate_feed_items_from_dict():
    test_mutate_feed_items(request_type=dict)
...
def test_feed_item_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v9.services.services.feed_item_service.transports.FeedItemServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FeedItemServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.FeedItemServiceGrpcTransport])
