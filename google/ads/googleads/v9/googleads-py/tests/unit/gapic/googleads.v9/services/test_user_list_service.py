...
def test_user_list_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v9.services.services.user_list_service.UserListServiceClient.get_transport_class') as gtc:
...
def test_user_list_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v9.services.services.user_list_service.transports.UserListServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UserListServiceTransport()
        adc.assert_called_once()
