...
def test_customer_client_link_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v9.services.services.customer_client_link_service.CustomerClientLinkServiceClient.get_transport_class') as gtc:
...
def test_mutate_customer_client_link_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].operation == customer_client_link_service.CustomerClientLinkOperation(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
...
def test_customer_client_link_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v9.services.services.customer_client_link_service.transports.CustomerClientLinkServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CustomerClientLinkServiceTransport()
        adc.assert_called_once()
