...
def test_ad_group_feed_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v9.services.services.ad_group_feed_service.AdGroupFeedServiceClient.get_transport_class') as gtc:
...
def test_ad_group_feed_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v9.services.services.ad_group_feed_service.transports.AdGroupFeedServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AdGroupFeedServiceTransport()
        adc.assert_called_once()
