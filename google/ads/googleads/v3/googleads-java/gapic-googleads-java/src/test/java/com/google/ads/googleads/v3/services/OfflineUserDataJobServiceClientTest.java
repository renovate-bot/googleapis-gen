...
public class OfflineUserDataJobServiceClientTest {
  ...
  @Test
  public void getOfflineUserDataJobTest() throws Exception {
    OfflineUserDataJob expectedResponse =
        OfflineUserDataJob.newBuilder()
            .setResourceName(
                OfflineUserDataJobName.of("[CUSTOMER]", "[OFFLINE_USER_DATA_JOB]").toString())
            .setId(Int64Value.newBuilder().build())
            .setExternalId(Int64Value.newBuilder().build())
            .build();
    mockOfflineUserDataJobService.addResponse(expectedResponse);

    OfflineUserDataJobName resourceName =
        OfflineUserDataJobName.of("[CUSTOMER]", "[OFFLINE_USER_DATA_JOB]");

    OfflineUserDataJob actualResponse = client.getOfflineUserDataJob(resourceName);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockOfflineUserDataJobService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    GetOfflineUserDataJobRequest actualRequest =
        ((GetOfflineUserDataJobRequest) actualRequests.get(0));
  ...
  @Test
  public void getOfflineUserDataJobExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockOfflineUserDataJobService.addException(exception);
    ...
    try {
      OfflineUserDataJobName resourceName =
          OfflineUserDataJobName.of("[CUSTOMER]", "[OFFLINE_USER_DATA_JOB]");
      client.getOfflineUserDataJob(resourceName);
      Assert.fail("No exception raised");
    } catch (InvalidArgumentException e) {
      // Expected exception.
    }
  ...
  @Test
  public void getOfflineUserDataJobTest2() throws Exception {
    OfflineUserDataJob expectedResponse =
        OfflineUserDataJob.newBuilder()
            .setResourceName(
                OfflineUserDataJobName.of("[CUSTOMER]", "[OFFLINE_USER_DATA_JOB]").toString())
  ...
  @Test
  public void getOfflineUserDataJobExceptionTest2() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockOfflineUserDataJobService.addException(exception);
