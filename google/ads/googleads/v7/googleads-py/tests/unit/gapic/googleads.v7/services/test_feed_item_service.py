...
from google.ads.googleads.v7.services.services.feed_item_service import FeedItemServiceClient
from google.ads.googleads.v7.services.services.feed_item_service import transports
...
def test_feed_item_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.FeedItemServiceGrpcTransport, "grpc"),
])
def test_feed_item_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_feed_item_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
def test_feed_item_service_client_get_transport_class():
    transport = FeedItemServiceClient.get_transport_class()
    available_transports = [
        transports.FeedItemServiceGrpcTransport,
    ]
    assert transport in available_transports

    transport = FeedItemServiceClient.get_transport_class("grpc")
    assert transport == transports.FeedItemServiceGrpcTransport


@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (FeedItemServiceClient, transports.FeedItemServiceGrpcTransport, "grpc"),
])
@mock.patch.object(FeedItemServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FeedItemServiceClient))
def test_feed_item_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(FeedItemServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (FeedItemServiceClient, transports.FeedItemServiceGrpcTransport, "grpc", "true"),
    (FeedItemServiceClient, transports.FeedItemServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(FeedItemServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FeedItemServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_feed_item_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_feed_item_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_feed_item_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_feed_item_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v7.services.services.feed_item_service.transports.FeedItemServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_feed_item(request_type, transport: str = 'grpc'):
    client = FeedItemServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_feed_item_empty_call():
...
def test_mutate_feed_items(request_type, transport: str = 'grpc'):
    client = FeedItemServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_mutate_feed_items_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = FeedItemServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_mutate_feed_items_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = [feed_item_service.FeedItemOperation(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
        assert arg == mock_val
...
def test_feed_item_service_base_transport_error():
...
def test_feed_item_service_base_transport():
    # Instantiate the base transport.
    with mock.patch('google.ads.googleads.v7.services.services.feed_item_service.transports.FeedItemServiceTransport.__init__') as Transport:
...
def test_feed_item_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.feed_item_service.transports.FeedItemServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_feed_item_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.feed_item_service.transports.FeedItemServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FeedItemServiceTransport()
        adc.assert_called_once()
...
def test_feed_item_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.FeedItemServiceGrpcTransport, grpc_helpers),
    ],
)
def test_feed_item_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_feed_item_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_feed_item_service_grpc_transport_channel():
...
def test_feed_item_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
def test_feed_item_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
