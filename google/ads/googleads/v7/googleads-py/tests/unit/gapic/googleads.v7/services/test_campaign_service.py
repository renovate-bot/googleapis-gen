...
from google.ads.googleads.v7.services.services.campaign_service import CampaignServiceClient
from google.ads.googleads.v7.services.services.campaign_service import transports
...
def test_campaign_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.CampaignServiceGrpcTransport, "grpc"),
])
def test_campaign_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_campaign_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (CampaignServiceClient, transports.CampaignServiceGrpcTransport, "grpc"),
])
@mock.patch.object(CampaignServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(CampaignServiceClient))
def test_campaign_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(CampaignServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (CampaignServiceClient, transports.CampaignServiceGrpcTransport, "grpc", "true"),
    (CampaignServiceClient, transports.CampaignServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(CampaignServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(CampaignServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_campaign_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_campaign_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_campaign_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_campaign_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v7.services.services.campaign_service.transports.CampaignServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_campaign(request_type, transport: str = 'grpc'):
    client = CampaignServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_campaign_empty_call():
...
def test_mutate_campaigns(request_type, transport: str = 'grpc'):
    client = CampaignServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_mutate_campaigns_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = CampaignServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_mutate_campaigns_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = [campaign_service.CampaignOperation(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
        assert arg == mock_val
...
def test_campaign_service_base_transport_error():
...
def test_campaign_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.campaign_service.transports.CampaignServiceTransport.__init__') as Transport:
...
def test_campaign_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.campaign_service.transports.CampaignServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_campaign_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.campaign_service.transports.CampaignServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CampaignServiceTransport()
        adc.assert_called_once()
...
def test_campaign_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.CampaignServiceGrpcTransport, grpc_helpers),
    ],
)
def test_campaign_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_campaign_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
