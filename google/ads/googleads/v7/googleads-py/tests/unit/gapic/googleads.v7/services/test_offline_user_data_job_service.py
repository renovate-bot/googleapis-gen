...
def test_offline_user_data_job_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.offline_user_data_job_service.transports.OfflineUserDataJobServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_offline_user_data_job_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.offline_user_data_job_service.transports.OfflineUserDataJobServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.OfflineUserDataJobServiceTransport()
        adc.assert_called_once()
