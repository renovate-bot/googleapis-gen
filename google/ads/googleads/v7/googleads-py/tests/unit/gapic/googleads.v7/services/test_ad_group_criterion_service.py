...
def test_ad_group_criterion_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.ad_group_criterion_service.AdGroupCriterionServiceClient.get_transport_class') as gtc:
...
def test_ad_group_criterion_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.ad_group_criterion_service.transports.AdGroupCriterionServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AdGroupCriterionServiceTransport()
        adc.assert_called_once()
