...
def test_feed_mapping_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.FeedMappingServiceClient.get_transport_class') as gtc:
...
def test_feed_mapping_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.transports.FeedMappingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FeedMappingServiceTransport()
        adc.assert_called_once()
