...
from google.ads.googleads.v7.services.services.feed_mapping_service import FeedMappingServiceClient
from google.ads.googleads.v7.services.services.feed_mapping_service import transports
...
def test_feed_mapping_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.FeedMappingServiceGrpcTransport, "grpc"),
])
def test_feed_mapping_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_feed_mapping_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (FeedMappingServiceClient, transports.FeedMappingServiceGrpcTransport, "grpc"),
])
@mock.patch.object(FeedMappingServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FeedMappingServiceClient))
def test_feed_mapping_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(FeedMappingServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (FeedMappingServiceClient, transports.FeedMappingServiceGrpcTransport, "grpc", "true"),
    (FeedMappingServiceClient, transports.FeedMappingServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(FeedMappingServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FeedMappingServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_feed_mapping_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_feed_mapping_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_feed_mapping_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_feed_mapping_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.transports.FeedMappingServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_feed_mapping(request_type, transport: str = 'grpc'):
    client = FeedMappingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_feed_mapping_empty_call():
...
def test_mutate_feed_mappings(request_type, transport: str = 'grpc'):
    client = FeedMappingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_mutate_feed_mappings_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = FeedMappingServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_mutate_feed_mappings_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = [feed_mapping_service.FeedMappingOperation(create=feed_mapping.FeedMapping(resource_name='resource_name_value'))]
        assert arg == mock_val
...
def test_feed_mapping_service_base_transport_error():
...
def test_feed_mapping_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.transports.FeedMappingServiceTransport.__init__') as Transport:
...
def test_feed_mapping_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.transports.FeedMappingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_feed_mapping_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.feed_mapping_service.transports.FeedMappingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FeedMappingServiceTransport()
        adc.assert_called_once()
...
def test_feed_mapping_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.FeedMappingServiceGrpcTransport, grpc_helpers),
    ],
)
def test_feed_mapping_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_feed_mapping_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
def test_feed_path():
    ...
    actual = FeedMappingServiceClient.feed_path(customer_id, feed_id)
...
def test_feed_mapping_path():
    ...
    actual = FeedMappingServiceClient.feed_mapping_path(customer_id, feed_id, feed_mapping_id)
    assert expected == actual
