...
def test_feed_item_set_service_client_from_service_account_info():
    creds = credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        factory.return_value = creds
        info = {"valid": True}
        client = FeedItemSetServiceClient.from_service_account_info(info)
        assert client.transport._credentials == creds

        assert client.transport._host == 'googleads.googleapis.com:443'


def test_feed_item_set_service_client_from_service_account_file():
    creds = credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_file') as factory:
        factory.return_value = creds
        client = FeedItemSetServiceClient.from_service_account_file("dummy/file/path.json")
        assert client.transport._credentials == creds

        client = FeedItemSetServiceClient.from_service_account_json("dummy/file/path.json")
        assert client.transport._credentials == creds
...
def test_feed_item_set_service_client_client_options():
    # Check that if channel is provided we won't create a new one.
    with mock.patch('google.ads.googleads.v7.services.services.feed_item_set_service.FeedItemSetServiceClient.get_transport_class') as gtc:
    ...
    with mock.patch('google.ads.googleads.v7.services.services.feed_item_set_service.FeedItemSetServiceClient.get_transport_class') as gtc:
        client = FeedItemSetServiceClient(transport="grpc")
        gtc.assert_called()
    ...
    with mock.patch('google.ads.googleads.v7.services.services.feed_item_set_service.transports.FeedItemSetServiceGrpcTransport.__init__') as grpc_transport:
        ...
        client = FeedItemSetServiceClient(client_options=options)
...
def test_feed_item_set_service_client_mtls_env_auto(use_client_cert_env):
...
def test_get_feed_item_set(transport: str = 'grpc', request_type=feed_item_set_service.GetFeedItemSetRequest):
    client = FeedItemSetServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_feed_item_set_from_dict():
    test_get_feed_item_set(request_type=dict)
...
def test_mutate_feed_item_sets(transport: str = 'grpc', request_type=feed_item_set_service.MutateFeedItemSetsRequest):
    client = FeedItemSetServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_mutate_feed_item_sets_from_dict():
    test_mutate_feed_item_sets(request_type=dict)
...
def test_mutate_feed_item_sets_flattened():
    ...
    with mock.patch.object(
        ...
        client.mutate_feed_item_sets(
            ...
            operations=[feed_item_set_service.FeedItemSetOperation(update_mask=field_mask.FieldMask(paths=['paths_value']))],
        )
        ...
        assert args[0].operations == [feed_item_set_service.FeedItemSetOperation(update_mask=field_mask.FieldMask(paths=['paths_value']))]
...
def test_feed_item_set_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.feed_item_set_service.transports.FeedItemSetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.FeedItemSetServiceTransport()
        adc.assert_called_once()
...
def test_feed_item_set_service_transport_auth_adc():
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, 'default') as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transports.FeedItemSetServiceGrpcTransport(host="squid.clam.whelk")
        adc.assert_called_once_with(scopes=(
            'https://www.googleapis.com/auth/adwords',
        ))
...
@pytest.mark.parametrize("transport_class", [transports.FeedItemSetServiceGrpcTransport])
