...
def test_mutate_feed_item_sets_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].operations == [feed_item_set_service.FeedItemSetOperation(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
...
def test_feed_item_set_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.feed_item_set_service.transports.FeedItemSetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FeedItemSetServiceTransport()
        adc.assert_called_once()
