...
from google.ads.googleads.v7.services.services.user_interest_service import UserInterestServiceClient
from google.ads.googleads.v7.services.services.user_interest_service import transports
...
def test_user_interest_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.UserInterestServiceGrpcTransport, "grpc"),
])
def test_user_interest_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_user_interest_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (UserInterestServiceClient, transports.UserInterestServiceGrpcTransport, "grpc"),
])
@mock.patch.object(UserInterestServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(UserInterestServiceClient))
def test_user_interest_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(UserInterestServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (UserInterestServiceClient, transports.UserInterestServiceGrpcTransport, "grpc", "true"),
    (UserInterestServiceClient, transports.UserInterestServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(UserInterestServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(UserInterestServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_user_interest_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_user_interest_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_user_interest_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_user_interest_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v7.services.services.user_interest_service.transports.UserInterestServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_user_interest(request_type, transport: str = 'grpc'):
    client = UserInterestServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_user_interest_empty_call():
...
def test_user_interest_service_base_transport_error():
...
def test_user_interest_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.user_interest_service.transports.UserInterestServiceTransport.__init__') as Transport:
...
def test_user_interest_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.user_interest_service.transports.UserInterestServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_user_interest_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.user_interest_service.transports.UserInterestServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UserInterestServiceTransport()
        adc.assert_called_once()
...
def test_user_interest_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.UserInterestServiceGrpcTransport, grpc_helpers),
    ],
)
def test_user_interest_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_user_interest_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
