...
def test_user_interest_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.user_interest_service.UserInterestServiceClient.get_transport_class') as gtc:
...
def test_user_interest_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.user_interest_service.transports.UserInterestServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.UserInterestServiceTransport()
        adc.assert_called_once()
