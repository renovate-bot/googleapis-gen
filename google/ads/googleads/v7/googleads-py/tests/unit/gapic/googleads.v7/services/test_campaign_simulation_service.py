...
def test_campaign_simulation_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.campaign_simulation_service.transports.CampaignSimulationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.CampaignSimulationServiceTransport()
        adc.assert_called_once()
