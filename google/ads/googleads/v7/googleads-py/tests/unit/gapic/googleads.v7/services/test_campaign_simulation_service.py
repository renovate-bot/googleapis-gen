...
def test_campaign_simulation_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.CampaignSimulationServiceGrpcTransport, "grpc"),
])
def test_campaign_simulation_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_campaign_simulation_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (CampaignSimulationServiceClient, transports.CampaignSimulationServiceGrpcTransport, "grpc"),
])
...
def test_campaign_simulation_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(CampaignSimulationServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (CampaignSimulationServiceClient, transports.CampaignSimulationServiceGrpcTransport, "grpc", "true"),
    (CampaignSimulationServiceClient, transports.CampaignSimulationServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(CampaignSimulationServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(CampaignSimulationServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_campaign_simulation_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_campaign_simulation_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_campaign_simulation_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_get_campaign_simulation(request_type, transport: str = 'grpc'):
    client = CampaignSimulationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_campaign_simulation_empty_call():
...
def test_campaign_simulation_service_base_transport_error():
...
def test_campaign_simulation_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.campaign_simulation_service.transports.CampaignSimulationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_campaign_simulation_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.campaign_simulation_service.transports.CampaignSimulationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CampaignSimulationServiceTransport()
        adc.assert_called_once()
...
def test_campaign_simulation_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.CampaignSimulationServiceGrpcTransport, grpc_helpers),
    ],
)
def test_campaign_simulation_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_campaign_simulation_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
