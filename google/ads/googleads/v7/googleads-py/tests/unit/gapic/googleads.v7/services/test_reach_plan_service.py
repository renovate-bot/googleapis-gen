...
from google.ads.googleads.v7.enums.types import reach_plan_ad_length
from google.ads.googleads.v7.enums.types import reach_plan_age_range
from google.ads.googleads.v7.enums.types import reach_plan_network
from google.ads.googleads.v7.services.services.reach_plan_service import ReachPlanServiceClient
from google.ads.googleads.v7.services.services.reach_plan_service import transports
...
def test_reach_plan_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.ReachPlanServiceGrpcTransport, "grpc"),
])
def test_reach_plan_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_reach_plan_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ReachPlanServiceClient, transports.ReachPlanServiceGrpcTransport, "grpc"),
])
@mock.patch.object(ReachPlanServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ReachPlanServiceClient))
def test_reach_plan_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(ReachPlanServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (ReachPlanServiceClient, transports.ReachPlanServiceGrpcTransport, "grpc", "true"),
    (ReachPlanServiceClient, transports.ReachPlanServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(ReachPlanServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ReachPlanServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_reach_plan_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_reach_plan_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_reach_plan_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_reach_plan_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v7.services.services.reach_plan_service.transports.ReachPlanServiceGrpcTransport.__init__') as grpc_transport:
...
def test_list_plannable_locations(request_type, transport: str = 'grpc'):
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_plannable_locations_empty_call():
    ...
    with mock.patch.object(
            type(client.transport.list_plannable_locations),
            '__call__') as call:
        client.list_plannable_locations()
        call.assert_called()
        _, args, _ = call.mock_calls[0]
        assert args[0] == reach_plan_service.ListPlannableLocationsRequest()
...
def test_list_plannable_products(request_type, transport: str = 'grpc'):
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_plannable_products_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_list_plannable_products_flattened():
    ...
    with mock.patch.object(
        ...
        assert len(call.mock_calls) == 1
        _, args, _ = call.mock_calls[0]
        arg = args[0].plannable_location_id
        mock_val = 'plannable_location_id_value'
        assert arg == mock_val
...
def test_generate_product_mix_ideas(request_type, transport: str = 'grpc'):
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_generate_product_mix_ideas_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_generate_product_mix_ideas_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = 'plannable_location_id_value'
        assert arg == mock_val
        arg = args[0].currency_code
        mock_val = 'currency_code_value'
        assert arg == mock_val
        arg = args[0].budget_micros
        mock_val = 1383
        assert arg == mock_val
...
def test_generate_reach_forecast(request_type, transport: str = 'grpc'):
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_generate_reach_forecast_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = ReachPlanServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_generate_reach_forecast_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = 'customer_id_value'
        assert arg == mock_val
        arg = args[0].campaign_duration
        mock_val = reach_plan_service.CampaignDuration(duration_in_days=1708)
        assert arg == mock_val
        arg = args[0].planned_products
        mock_val = [reach_plan_service.PlannedProduct(plannable_product_code='plannable_product_code_value')]
        assert arg == mock_val
...
def test_reach_plan_service_base_transport_error():
...
def test_reach_plan_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.reach_plan_service.transports.ReachPlanServiceTransport.__init__') as Transport:
...
def test_reach_plan_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v7.services.services.reach_plan_service.transports.ReachPlanServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_reach_plan_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v7.services.services.reach_plan_service.transports.ReachPlanServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ReachPlanServiceTransport()
        adc.assert_called_once()
...
def test_reach_plan_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.ReachPlanServiceGrpcTransport, grpc_helpers),
    ],
)
def test_reach_plan_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_reach_plan_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
