...
def test_hotel_performance_view_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.hotel_performance_view_service.transports.HotelPerformanceViewServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.HotelPerformanceViewServiceTransport()
        adc.assert_called_once()
