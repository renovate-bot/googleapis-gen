...
def test_recommendation_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v7.services.services.recommendation_service.RecommendationServiceClient.get_transport_class') as gtc:
...
def test_recommendation_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v7.services.services.recommendation_service.transports.RecommendationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.RecommendationServiceTransport()
        adc.assert_called_once()
