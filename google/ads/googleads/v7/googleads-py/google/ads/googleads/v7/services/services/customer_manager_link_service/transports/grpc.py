...
class CustomerManagerLinkServiceGrpcTransport(CustomerManagerLinkServiceTransport):
    """gRPC backend transport for CustomerManagerLinkService.
    ...
    def get_customer_manager_link(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetCustomerManagerLinkRequest],
                    ~.CustomerManagerLink]:
        ...
        if 'get_customer_manager_link' not in self._stubs:
            self._stubs['get_customer_manager_link'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v7.services.CustomerManagerLinkService/GetCustomerManagerLink',
                request_serializer=customer_manager_link_service.GetCustomerManagerLinkRequest.serialize,
                response_deserializer=customer_manager_link.CustomerManagerLink.deserialize,
            )
        return self._stubs['get_customer_manager_link']
    ...
    def mutate_customer_manager_link(self) -> Callable[
        ...
        if 'mutate_customer_manager_link' not in self._stubs:
            self._stubs['mutate_customer_manager_link'] = self.grpc_channel.unary_unary(
    ...
    def move_manager_link(self) -> Callable[
        ...
        if 'move_manager_link' not in self._stubs:
            self._stubs['move_manager_link'] = self.grpc_channel.unary_unary(
