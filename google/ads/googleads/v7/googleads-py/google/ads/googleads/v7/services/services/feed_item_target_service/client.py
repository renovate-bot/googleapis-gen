...
class FeedItemTargetServiceClientMeta(type):
    ...
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[FeedItemTargetServiceTransport]]
...
class FeedItemTargetServiceClient(metaclass=FeedItemTargetServiceClientMeta):
    ...
    def transport(self) -> FeedItemTargetServiceTransport:
        ...
        Returns:
            FeedItemTargetServiceTransport: The transport used by the client
                instance.
        """
    ...
    def __init__(self, *,
        ...
        """Instantiates the feed item target service client.
    ...
    def get_feed_item_target(self,
        ...
        r"""Returns the requested feed item targets in full detail.
        ...
        Returns:
            google.ads.googleads.v7.resources.types.FeedItemTarget:
                A feed item target.
        """
        ...
        if not isinstance(request, feed_item_target_service.GetFeedItemTargetRequest):
        ...
        rpc = self._transport._wrapped_methods[self._transport.get_feed_item_target]
    ...
    def mutate_feed_item_targets(self,
        ...
        r"""Creates or removes feed item targets. Operation statuses are
        returned.

        List of thrown errors: `AuthenticationError <>`__
        `AuthorizationError <>`__ `CriterionError <>`__
        `DatabaseError <>`__ `DistinctError <>`__
        `FeedItemTargetError <>`__ `FieldError <>`__ `HeaderError <>`__
        ...
        Returns:
            google.ads.googleads.v7.services.types.MutateFeedItemTargetsResponse:
        ...
        # There's no risk of modifying the input as we've already verified
        # there are no flattened fields.
        if not isinstance(request, feed_item_target_service.MutateFeedItemTargetsRequest):
            request = feed_item_target_service.MutateFeedItemTargetsRequest(request)
             # If we have keyword arguments corresponding to fields on the
            # request, apply these.
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.mutate_feed_item_targets]
