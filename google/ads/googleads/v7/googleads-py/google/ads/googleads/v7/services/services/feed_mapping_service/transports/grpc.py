...
from .base import FeedMappingServiceTransport, DEFAULT_CLIENT_INFO


class FeedMappingServiceGrpcTransport(FeedMappingServiceTransport):
    """gRPC backend transport for FeedMappingService.
    ...
    @property
    def get_feed_mapping(self) -> Callable[
            [feed_mapping_service.GetFeedMappingRequest],
            feed_mapping.FeedMapping]:
        r"""Return a callable for the get feed mapping method over gRPC.
        ...
        Returns:
            Callable[[~.GetFeedMappingRequest],
                    ~.FeedMapping]:
        ...
        if 'get_feed_mapping' not in self._stubs:
            self._stubs['get_feed_mapping'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v7.services.FeedMappingService/GetFeedMapping',
                request_serializer=feed_mapping_service.GetFeedMappingRequest.serialize,
                response_deserializer=feed_mapping.FeedMapping.deserialize,
            )
        return self._stubs['get_feed_mapping']

    @property
    def mutate_feed_mappings(self) -> Callable[
            [feed_mapping_service.MutateFeedMappingsRequest],
            feed_mapping_service.MutateFeedMappingsResponse]:
        r"""Return a callable for the mutate feed mappings method over gRPC.
        ...
        Returns:
            Callable[[~.MutateFeedMappingsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_feed_mappings' not in self._stubs:
            self._stubs['mutate_feed_mappings'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v7.services.FeedMappingService/MutateFeedMappings',
                request_serializer=feed_mapping_service.MutateFeedMappingsRequest.serialize,
                response_deserializer=feed_mapping_service.MutateFeedMappingsResponse.deserialize,
            )
        return self._stubs['mutate_feed_mappings']
