...
from .base import AdGroupAssetServiceTransport, DEFAULT_CLIENT_INFO


class AdGroupAssetServiceGrpcTransport(AdGroupAssetServiceTransport):
    """gRPC backend transport for AdGroupAssetService.
    ...
    @property
    def get_ad_group_asset(self) -> Callable[
            [ad_group_asset_service.GetAdGroupAssetRequest],
            ad_group_asset.AdGroupAsset]:
        r"""Return a callable for the get ad group asset method over gRPC.
        ...
        Returns:
            Callable[[~.GetAdGroupAssetRequest],
                    ~.AdGroupAsset]:
        ...
        if 'get_ad_group_asset' not in self._stubs:
            self._stubs['get_ad_group_asset'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v7.services.AdGroupAssetService/GetAdGroupAsset',
                request_serializer=ad_group_asset_service.GetAdGroupAssetRequest.serialize,
                response_deserializer=ad_group_asset.AdGroupAsset.deserialize,
            )
        return self._stubs['get_ad_group_asset']

    @property
    def mutate_ad_group_assets(self) -> Callable[
            [ad_group_asset_service.MutateAdGroupAssetsRequest],
            ad_group_asset_service.MutateAdGroupAssetsResponse]:
        r"""Return a callable for the mutate ad group assets method over gRPC.

        Creates, updates, or removes ad group assets. Operation statuses
        are returned.
        ...
        Returns:
            Callable[[~.MutateAdGroupAssetsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_ad_group_assets' not in self._stubs:
            self._stubs['mutate_ad_group_assets'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v7.services.AdGroupAssetService/MutateAdGroupAssets',
                request_serializer=ad_group_asset_service.MutateAdGroupAssetsRequest.serialize,
                response_deserializer=ad_group_asset_service.MutateAdGroupAssetsResponse.deserialize,
            )
        return self._stubs['mutate_ad_group_assets']
