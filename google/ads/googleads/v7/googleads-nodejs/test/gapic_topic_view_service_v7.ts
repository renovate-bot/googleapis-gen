...
import * as topicviewserviceModule from '../src';
...
describe('v7.TopicViewServiceClient', () => {
    it('has servicePath', () => {
        const servicePath = topicviewserviceModule.v7.TopicViewServiceClient.servicePath;
        assert(servicePath);
    });

    it('has apiEndpoint', () => {
        const apiEndpoint = topicviewserviceModule.v7.TopicViewServiceClient.apiEndpoint;
        assert(apiEndpoint);
    });

    it('has port', () => {
        const port = topicviewserviceModule.v7.TopicViewServiceClient.port;
        assert(port);
        assert(typeof port === 'number');
    });

    it('should create a client with no option', () => {
        const client = new topicviewserviceModule.v7.TopicViewServiceClient();
        assert(client);
    });
    ...
    it('has initialize method and supports deferred initialization', async () => {
        ...
        assert.strictEqual(client.topicViewServiceStub, undefined);
        await client.initialize();
        assert(client.topicViewServiceStub);
    });
    ...
    describe('getTopicView', () => {
        it('invokes getTopicView without error', async () => {
            const client = new topicviewserviceModule.v7.TopicViewServiceClient({
              credentials: {client_email: 'bogus', private_key: 'bogus'},
              projectId: 'bogus',
        });
            client.initialize();
            const request = generateSampleMessage(new protos.google.ads.googleads.v7.services.GetTopicViewRequest());
            ...
            const expectedResponse = generateSampleMessage(new protos.google.ads.googleads.v7.resources.TopicView());
            client.innerApiCalls.getTopicView = stubSimpleCall(expectedResponse);
            const [response] = await client.getTopicView(request);
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.getTopicView as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });

        it('invokes getTopicView without error using callback', async () => {
            const client = new topicviewserviceModule.v7.TopicViewServiceClient({
              credentials: {client_email: 'bogus', private_key: 'bogus'},
              projectId: 'bogus',
        });
            ...
            const expectedResponse = generateSampleMessage(new protos.google.ads.googleads.v7.resources.TopicView());
            client.innerApiCalls.getTopicView = stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
                 client.getTopicView(
                    request,
                    (err?: Error|null, result?: protos.google.ads.googleads.v7.resources.ITopicView|null) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
        ...
        it('invokes getTopicView with error', async () => {
            const client = new topicviewserviceModule.v7.TopicViewServiceClient({
              credentials: {client_email: 'bogus', private_key: 'bogus'},
              projectId: 'bogus',
        });
            ...
            client.innerApiCalls.getTopicView = stubSimpleCall(undefined, expectedError);
            await assert.rejects(client.getTopicView(request), expectedError);
            assert((client.innerApiCalls.getTopicView as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    });
