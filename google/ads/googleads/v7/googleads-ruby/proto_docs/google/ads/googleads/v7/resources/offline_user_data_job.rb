# frozen_string_literal: true

# Copyright 2022 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# Auto-generated by gapic-generator-ruby. DO NOT EDIT!


module Google
  module Ads
    module GoogleAds
      module V7
        module Resources
          # A job containing offline user data of store visitors, or user list members
          # that will be processed asynchronously. The uploaded data isn't readable and
          # the processing results of the job can only be read using
          # OfflineUserDataJobService.GetOfflineUserDataJob.
          # @!attribute [rw] resource_name
          #   @return [::String]
          #     Immutable. The resource name of the offline user data job.
          #     Offline user data job resource names have the form:
          #
          #     `customers/{customer_id}/offlineUserDataJobs/{offline_user_data_job_id}`
          # @!attribute [r] id
          #   @return [::Integer]
          #     Output only. ID of this offline user data job.
          # @!attribute [rw] external_id
          #   @return [::Integer]
          #     Immutable. User specified job ID.
          # @!attribute [rw] type
          #   @return [::Google::Ads::GoogleAds::V7::Enums::OfflineUserDataJobTypeEnum::OfflineUserDataJobType]
          #     Immutable. Type of the job.
          # @!attribute [r] status
          #   @return [::Google::Ads::GoogleAds::V7::Enums::OfflineUserDataJobStatusEnum::OfflineUserDataJobStatus]
          #     Output only. Status of the job.
          # @!attribute [r] failure_reason
          #   @return [::Google::Ads::GoogleAds::V7::Enums::OfflineUserDataJobFailureReasonEnum::OfflineUserDataJobFailureReason]
          #     Output only. Reason for the processing failure, if status is FAILED.
          # @!attribute [rw] customer_match_user_list_metadata
          #   @return [::Google::Ads::GoogleAds::V7::Common::CustomerMatchUserListMetadata]
          #     Immutable. Metadata for data updates to a CRM-based user list.
          # @!attribute [rw] store_sales_metadata
          #   @return [::Google::Ads::GoogleAds::V7::Common::StoreSalesMetadata]
          #     Immutable. Metadata for store sales data update.
          class OfflineUserDataJob
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end
        end
      end
    end
  end
end
