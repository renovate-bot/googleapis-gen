# frozen_string_literal: true

# Copyright 2021 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# Auto-generated by gapic-generator-ruby. DO NOT EDIT!


module Google
  module Ads
    module GoogleAds
      module V7
        module Resources
          # A call view that includes data for call tracking of call-only ads or call
          # extensions.
          # @!attribute [r] resource_name
          #   @return [::String]
          #     Output only. The resource name of the call view.
          #     Call view resource names have the form:
          #
          #     `customers/{customer_id}/callViews/{call_detail_id}`
          # @!attribute [r] caller_region_code
          #   @return [::String]
          #     Output only. Region code of the caller.
          # @!attribute [r] caller_area_code
          #   @return [::String]
          #     Output only. Area code of the caller. Null if the call duration is shorter than 15
          #     seconds.
          # @!attribute [r] call_duration_seconds
          #   @return [::Integer]
          #     Output only. The advertiser-provided call duration in seconds.
          # @!attribute [r] start_call_date_time
          #   @return [::String]
          #     Output only. The advertiser-provided call start date time.
          # @!attribute [r] end_call_date_time
          #   @return [::String]
          #     Output only. The advertiser-provided call end date time.
          # @!attribute [r] call_tracking_display_location
          #   @return [::Google::Ads::GoogleAds::V7::Enums::CallTrackingDisplayLocationEnum::CallTrackingDisplayLocation]
          #     Output only. The call tracking display location.
          # @!attribute [r] type
          #   @return [::Google::Ads::GoogleAds::V7::Enums::CallTypeEnum::CallType]
          #     Output only. The type of the call.
          # @!attribute [r] call_status
          #   @return [::Google::Ads::GoogleAds::V7::Enums::GoogleVoiceCallStatusEnum::GoogleVoiceCallStatus]
          #     Output only. The status of the call.
          class CallView
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end
        end
      end
    end
  end
end
