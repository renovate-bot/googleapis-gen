...
class AccountBudgetProposalServiceClientMeta(type):
    ...
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[AccountBudgetProposalServiceTransport]]
...
class AccountBudgetProposalServiceClient(metaclass=AccountBudgetProposalServiceClientMeta):
    ...
    def account_budget_path(customer: str,account_budget: str,) -> str:
        ...
        return "customers/{customer}/accountBudgets/{account_budget}".format(customer=customer, account_budget=account_budget, )

    @staticmethod
    def parse_account_budget_path(path: str) -> Dict[str,str]:
        """Parse a account_budget path into its component segments."""
        m = re.match(r"^customers/(?P<customer>.+?)/accountBudgets/(?P<account_budget>.+?)$", path)
        return m.groupdict() if m else {}
    @staticmethod
    def account_budget_proposal_path(customer: str,account_budget_proposal: str,) -> str:
        """Return a fully-qualified account_budget_proposal string."""
        return "customers/{customer}/accountBudgetProposals/{account_budget_proposal}".format(customer=customer, account_budget_proposal=account_budget_proposal, )
    ...
    def parse_account_budget_proposal_path(path: str) -> Dict[str,str]:
        ...
        m = re.match(r"^customers/(?P<customer>.+?)/accountBudgetProposals/(?P<account_budget_proposal>.+?)$", path)
        return m.groupdict() if m else {}
    @staticmethod
    def billing_setup_path(customer: str,billing_setup: str,) -> str:
        """Return a fully-qualified billing_setup string."""
        return "customers/{customer}/billingSetups/{billing_setup}".format(customer=customer, billing_setup=billing_setup, )

    @staticmethod
    def parse_billing_setup_path(path: str) -> Dict[str,str]:
        """Parse a billing_setup path into its component segments."""
        m = re.match(r"^customers/(?P<customer>.+?)/billingSetups/(?P<billing_setup>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def get_account_budget_proposal(self,
        ...
        rpc = self._transport._wrapped_methods[self._transport.get_account_budget_proposal]
    ...
    def mutate_account_budget_proposal(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.mutate_account_budget_proposal]
