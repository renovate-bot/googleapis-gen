...
def test_mutate_ads_flattened():
    ...
    with mock.patch.object(
        ...
        client.mutate_ads(
            ...
            operations=[ad_service.AdOperation(update_mask=field_mask.FieldMask(paths=['paths_value']))],
        )
        ...
        assert args[0].operations == [ad_service.AdOperation(update_mask=field_mask.FieldMask(paths=['paths_value']))]
...
def test_ad_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v5.services.services.ad_service.transports.AdServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AdServiceTransport()
        adc.assert_called_once()
