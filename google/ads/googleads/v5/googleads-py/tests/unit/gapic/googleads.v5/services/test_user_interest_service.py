...
def test_user_interest_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v5.services.services.user_interest_service.transports.UserInterestServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.UserInterestServiceTransport()
        adc.assert_called_once()
