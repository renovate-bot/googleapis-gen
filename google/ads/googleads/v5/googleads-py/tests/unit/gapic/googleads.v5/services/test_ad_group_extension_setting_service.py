...
def test_ad_group_extension_setting_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v5.services.services.ad_group_extension_setting_service.transports.AdGroupExtensionSettingServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AdGroupExtensionSettingServiceTransport()
        adc.assert_called_once()
