...
def test_add_offline_user_data_job_operations_flattened():
    ...
    with mock.patch.object(
        ...
        client.add_offline_user_data_job_operations(
            resource_name='resource_name_value',
            operations=[offline_user_data_job_service.OfflineUserDataJobOperation(create=offline_user_data.UserData(user_identifiers=[offline_user_data.UserIdentifier(hashed_email=wrappers.StringValue(value='value_value'))]))],
        )
...
def test_offline_user_data_job_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v5.services.services.offline_user_data_job_service.transports.OfflineUserDataJobServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.OfflineUserDataJobServiceTransport()
        adc.assert_called_once()
