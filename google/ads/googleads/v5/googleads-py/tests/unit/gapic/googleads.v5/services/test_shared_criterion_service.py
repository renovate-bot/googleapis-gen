...
def test_shared_criterion_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v5.services.services.shared_criterion_service.transports.SharedCriterionServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.SharedCriterionServiceTransport()
        adc.assert_called_once()
