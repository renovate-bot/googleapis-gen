...
import com.google.ads.googleads.v5.services.stub.CampaignServiceStub;
import com.google.ads.googleads.v5.services.stub.CampaignServiceStubSettings;
...
public class CampaignServiceClient implements BackgroundResource {
   ...
   * <pre>{@code
   * try (CampaignServiceClient campaignServiceClient = CampaignServiceClient.create()) {
   *   String resourceName = CampaignName.of("[CUSTOMER]", "[CAMPAIGN]").toString();
   *   Campaign response = campaignServiceClient.getCampaign(resourceName);
   * }
   * }</pre>
   *
