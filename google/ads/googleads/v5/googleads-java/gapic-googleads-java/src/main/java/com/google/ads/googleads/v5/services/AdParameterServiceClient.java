...
import com.google.ads.googleads.v5.services.stub.AdParameterServiceStub;
import com.google.ads.googleads.v5.services.stub.AdParameterServiceStubSettings;
...
public class AdParameterServiceClient implements BackgroundResource {
   ...
   * <pre>{@code
   * try (AdParameterServiceClient adParameterServiceClient = AdParameterServiceClient.create()) {
   *   String resourceName = AdParameterName.of("[CUSTOMER]", "[AD_PARAMETER]").toString();
   *   AdParameter response = adParameterServiceClient.getAdParameter(resourceName);
   * }
   * }</pre>
   *
