# frozen_string_literal: true

# Copyright 2022 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# Auto-generated by gapic-generator-ruby. DO NOT EDIT!


module Google
  module Ads
    module GoogleAds
      module V8
        module Resources
          # A media file.
          # @!attribute [rw] resource_name
          #   @return [::String]
          #     Immutable. The resource name of the media file.
          #     Media file resource names have the form:
          #
          #     `customers/{customer_id}/mediaFiles/{media_file_id}`
          # @!attribute [r] id
          #   @return [::Integer]
          #     Output only. The ID of the media file.
          # @!attribute [rw] type
          #   @return [::Google::Ads::GoogleAds::V8::Enums::MediaTypeEnum::MediaType]
          #     Immutable. Type of the media file.
          # @!attribute [r] mime_type
          #   @return [::Google::Ads::GoogleAds::V8::Enums::MimeTypeEnum::MimeType]
          #     Output only. The mime type of the media file.
          # @!attribute [rw] source_url
          #   @return [::String]
          #     Immutable. The URL of where the original media file was downloaded from (or a file
          #     name). Only used for media of type AUDIO and IMAGE.
          # @!attribute [rw] name
          #   @return [::String]
          #     Immutable. The name of the media file. The name can be used by clients to help
          #     identify previously uploaded media.
          # @!attribute [r] file_size
          #   @return [::Integer]
          #     Output only. The size of the media file in bytes.
          # @!attribute [rw] image
          #   @return [::Google::Ads::GoogleAds::V8::Resources::MediaImage]
          #     Immutable. Encapsulates an Image.
          # @!attribute [rw] media_bundle
          #   @return [::Google::Ads::GoogleAds::V8::Resources::MediaBundle]
          #     Immutable. A ZIP archive media the content of which contains HTML5 assets.
          # @!attribute [r] audio
          #   @return [::Google::Ads::GoogleAds::V8::Resources::MediaAudio]
          #     Output only. Encapsulates an Audio.
          # @!attribute [rw] video
          #   @return [::Google::Ads::GoogleAds::V8::Resources::MediaVideo]
          #     Immutable. Encapsulates a Video.
          class MediaFile
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end

          # Encapsulates an Image.
          # @!attribute [rw] data
          #   @return [::String]
          #     Immutable. Raw image data.
          # @!attribute [r] full_size_image_url
          #   @return [::String]
          #     Output only. The url to the full size version of the image.
          # @!attribute [r] preview_size_image_url
          #   @return [::String]
          #     Output only. The url to the preview size version of the image.
          class MediaImage
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end

          # Represents a ZIP archive media the content of which contains HTML5 assets.
          # @!attribute [rw] data
          #   @return [::String]
          #     Immutable. Raw zipped data.
          # @!attribute [r] url
          #   @return [::String]
          #     Output only. The url to access the uploaded zipped data.
          #     E.g. https://tpc.googlesyndication.com/simgad/123
          #     This field is read-only.
          class MediaBundle
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end

          # Encapsulates an Audio.
          # @!attribute [r] ad_duration_millis
          #   @return [::Integer]
          #     Output only. The duration of the Audio in milliseconds.
          class MediaAudio
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end

          # Encapsulates a Video.
          # @!attribute [r] ad_duration_millis
          #   @return [::Integer]
          #     Output only. The duration of the Video in milliseconds.
          # @!attribute [rw] youtube_video_id
          #   @return [::String]
          #     Immutable. The YouTube video ID (as seen in YouTube URLs). Adding prefix
          #     "https://www.youtube.com/watch?v=" to this ID will get the YouTube
          #     streaming URL for this video.
          # @!attribute [r] advertising_id_code
          #   @return [::String]
          #     Output only. The Advertising Digital Identification code for this video, as defined by
          #     the American Association of Advertising Agencies, used mainly for
          #     television commercials.
          # @!attribute [r] isci_code
          #   @return [::String]
          #     Output only. The Industry Standard Commercial Identifier code for this video, used
          #     mainly for television commercials.
          class MediaVideo
            include ::Google::Protobuf::MessageExts
            extend ::Google::Protobuf::MessageExts::ClassMethods
          end
        end
      end
    end
  end
end
