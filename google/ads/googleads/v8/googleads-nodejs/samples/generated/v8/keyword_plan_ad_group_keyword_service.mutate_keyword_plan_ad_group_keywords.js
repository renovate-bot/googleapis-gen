...
function main(customerId, operations) {
  // [START googleads_mutate_keyword_plan_ad_group_keywords_sample]
  /**
  ...
  const {KeywordPlanAdGroupKeywordServiceClient} = require('google-ads').v8;
  ...
  async function mutateKeywordPlanAdGroupKeywords() {
    ...
    // Run request
    const response = await googleadsClient.mutateKeywordPlanAdGroupKeywords(request);
    console.log(response);
  }

  mutateKeywordPlanAdGroupKeywords();
  // [END googleads_mutate_keyword_plan_ad_group_keywords_sample]
}
