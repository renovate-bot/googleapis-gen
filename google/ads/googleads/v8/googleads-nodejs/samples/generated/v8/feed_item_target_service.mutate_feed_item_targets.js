...
function main(customerId, operations) {
  // [START googleads_v8_generated_FeedItemTargetService_MutateFeedItemTargets_async]
  /**
  ...
  const {FeedItemTargetServiceClient} = require('google-ads').v8;
  ...
  async function callMutateFeedItemTargets() {
    ...
    // Run request
    const response = await googleadsClient.mutateFeedItemTargets(request);
    console.log(response);
  }

  callMutateFeedItemTargets();
  // [END googleads_v8_generated_FeedItemTargetService_MutateFeedItemTargets_async]
}
