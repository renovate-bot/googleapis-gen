...
function main(customerId, operations) {
  // [START googleads_v8_generated_AdGroupLabelService_MutateAdGroupLabels_async]
  /**
  ...
  const {AdGroupLabelServiceClient} = require('google-ads').v8;

  // Instantiates a client
  const googleadsClient = new AdGroupLabelServiceClient();

  async function mutateAdGroupLabels() {
    // Construct request
    const request = {
      customerId,
      operations,
  ...
  mutateAdGroupLabels();
  // [END googleads_v8_generated_AdGroupLabelService_MutateAdGroupLabels_async]
}
