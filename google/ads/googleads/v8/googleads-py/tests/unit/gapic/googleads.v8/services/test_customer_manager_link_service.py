...
def test_customer_manager_link_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.customer_manager_link_service.CustomerManagerLinkServiceClient.get_transport_class') as gtc:
...
def test_customer_manager_link_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v8.services.services.customer_manager_link_service.transports.CustomerManagerLinkServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CustomerManagerLinkServiceTransport()
        adc.assert_called_once()
