...
from google.ads.googleads.v8.services.services.customer_feed_service import CustomerFeedServiceClient
from google.ads.googleads.v8.services.services.customer_feed_service import transports
...
def test_customer_feed_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.CustomerFeedServiceGrpcTransport, "grpc"),
])
def test_customer_feed_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_customer_feed_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (CustomerFeedServiceClient, transports.CustomerFeedServiceGrpcTransport, "grpc"),
])
@mock.patch.object(CustomerFeedServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(CustomerFeedServiceClient))
def test_customer_feed_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(CustomerFeedServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (CustomerFeedServiceClient, transports.CustomerFeedServiceGrpcTransport, "grpc", "true"),
    (CustomerFeedServiceClient, transports.CustomerFeedServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(CustomerFeedServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(CustomerFeedServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_customer_feed_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_customer_feed_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_customer_feed_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_customer_feed_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v8.services.services.customer_feed_service.transports.CustomerFeedServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_customer_feed(request_type, transport: str = 'grpc'):
    client = CustomerFeedServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_customer_feed_empty_call():
...
def test_mutate_customer_feeds(request_type, transport: str = 'grpc'):
    client = CustomerFeedServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_mutate_customer_feeds_empty_call():
    # This test is a coverage failsafe to make sure that totally empty calls,
    # i.e. request == None and no flattened fields passed, work.
    client = CustomerFeedServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_mutate_customer_feeds_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = [customer_feed_service.CustomerFeedOperation(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
        assert arg == mock_val
...
def test_customer_feed_service_base_transport_error():
...
def test_customer_feed_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.customer_feed_service.transports.CustomerFeedServiceTransport.__init__') as Transport:
...
def test_customer_feed_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v8.services.services.customer_feed_service.transports.CustomerFeedServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_customer_feed_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v8.services.services.customer_feed_service.transports.CustomerFeedServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CustomerFeedServiceTransport()
        adc.assert_called_once()
...
def test_customer_feed_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.CustomerFeedServiceGrpcTransport, grpc_helpers),
    ],
)
def test_customer_feed_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_customer_feed_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
