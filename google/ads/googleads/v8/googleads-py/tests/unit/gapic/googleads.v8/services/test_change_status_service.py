...
from google.ads.googleads.v8.enums.types import change_status_operation
from google.ads.googleads.v8.enums.types import change_status_resource_type
from google.ads.googleads.v8.resources.types import change_status
from google.ads.googleads.v8.services.services.change_status_service import ChangeStatusServiceClient
from google.ads.googleads.v8.services.services.change_status_service import transports
...
def test_change_status_service_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.ChangeStatusServiceGrpcTransport, "grpc"),
])
def test_change_status_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
def test_change_status_service_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ChangeStatusServiceClient, transports.ChangeStatusServiceGrpcTransport, "grpc"),
])
@mock.patch.object(ChangeStatusServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ChangeStatusServiceClient))
def test_change_status_service_client_client_options(client_class, transport_class, transport_name):
    # Check that if channel is provided we won't create a new one.
    with mock.patch.object(ChangeStatusServiceClient, 'get_transport_class') as gtc:
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (ChangeStatusServiceClient, transports.ChangeStatusServiceGrpcTransport, "grpc", "true"),
    (ChangeStatusServiceClient, transports.ChangeStatusServiceGrpcTransport, "grpc", "false"),
])
@mock.patch.object(ChangeStatusServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ChangeStatusServiceClient))
@mock.patch.dict(os.environ, {"GOOGLE_API_USE_MTLS_ENDPOINT": "auto"})
def test_change_status_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_change_status_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_change_status_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_change_status_service_client_client_options_from_dict():
    with mock.patch('google.ads.googleads.v8.services.services.change_status_service.transports.ChangeStatusServiceGrpcTransport.__init__') as grpc_transport:
...
def test_get_change_status(request_type, transport: str = 'grpc'):
    client = ChangeStatusServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_get_change_status_empty_call():
...
def test_change_status_service_base_transport_error():
...
def test_change_status_service_base_transport():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.change_status_service.transports.ChangeStatusServiceTransport.__init__') as Transport:
...
def test_change_status_service_base_transport_with_credentials_file():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.ads.googleads.v8.services.services.change_status_service.transports.ChangeStatusServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_change_status_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.ads.googleads.v8.services.services.change_status_service.transports.ChangeStatusServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ChangeStatusServiceTransport()
        adc.assert_called_once()
...
def test_change_status_service_transport_auth_adc(transport_class):
...
@pytest.mark.parametrize(
    ...
    [
        (transports.ChangeStatusServiceGrpcTransport, grpc_helpers),
    ],
)
def test_change_status_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
...
def test_change_status_service_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
