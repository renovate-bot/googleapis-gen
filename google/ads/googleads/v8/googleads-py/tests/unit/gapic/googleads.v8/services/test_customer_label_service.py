...
def test_customer_label_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.customer_label_service.CustomerLabelServiceClient.get_transport_class') as gtc:
...
def test_customer_label_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v8.services.services.customer_label_service.transports.CustomerLabelServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CustomerLabelServiceTransport()
        adc.assert_called_once()
