...
def test_ad_group_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.ad_group_service.AdGroupServiceClient.get_transport_class') as gtc:
...
def test_ad_group_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v8.services.services.ad_group_service.transports.AdGroupServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AdGroupServiceTransport()
        adc.assert_called_once()
