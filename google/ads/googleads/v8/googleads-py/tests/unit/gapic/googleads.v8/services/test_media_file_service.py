...
def test_media_file_service_client_client_options():
    ...
    with mock.patch('google.ads.googleads.v8.services.services.media_file_service.MediaFileServiceClient.get_transport_class') as gtc:
...
def test_media_file_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v8.services.services.media_file_service.transports.MediaFileServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.MediaFileServiceTransport()
        adc.assert_called_once()
