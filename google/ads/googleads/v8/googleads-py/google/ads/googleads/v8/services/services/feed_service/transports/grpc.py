...
class FeedServiceGrpcTransport(FeedServiceTransport):
    """gRPC backend transport for FeedService.
    ...
    def get_feed(self) -> Callable[
        ...
        if 'get_feed' not in self._stubs:
            self._stubs['get_feed'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=feed_service.GetFeedRequest.serialize,
                response_deserializer=feed.Feed.deserialize,
            )
    ...
    def mutate_feeds(self) -> Callable[
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_feeds' not in self._stubs:
            self._stubs['mutate_feeds'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v8.services.FeedService/MutateFeeds',
                request_serializer=feed_service.MutateFeedsRequest.serialize,
                response_deserializer=feed_service.MutateFeedsResponse.deserialize,
            )
        return self._stubs['mutate_feeds']
