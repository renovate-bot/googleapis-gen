...
import com.google.ads.googleads.v6.resources.ParentalStatusViewName;
import com.google.ads.googleads.v6.services.stub.ParentalStatusViewServiceStub;
...
public class ParentalStatusViewServiceClient implements BackgroundResource {
   ...
   * calls. This is for advanced usage - prefer using create(ParentalStatusViewServiceSettings).
   */
   ...
   *     ParentalStatusViewServiceClient.create()) {
   *   String resourceName =
   *       ParentalStatusViewName.of("[CUSTOMER_ID]", "[AD_GROUP_ID]", "[CRITERION_ID]").toString();
   *   ParentalStatusView response =
   *       parentalStatusViewServiceClient.getParentalStatusView(resourceName);
   * }
   * }</pre>
   ...
   *                   .toString())
   *           .build();
   *   ApiFuture<ParentalStatusView> future =
   *       parentalStatusViewServiceClient.getParentalStatusViewCallable().futureCall(request);
   *   // Do something.
