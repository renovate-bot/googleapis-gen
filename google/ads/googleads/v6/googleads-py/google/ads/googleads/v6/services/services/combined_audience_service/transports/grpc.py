...
class CombinedAudienceServiceGrpcTransport(CombinedAudienceServiceTransport):
    """gRPC backend transport for CombinedAudienceService.

    Service to manage combined audiences. This service can be
    used to list all your combined audiences with metadata, but
    won't show the structure and components of the combined
    audience.
    ...
    def get_combined_audience(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetCombinedAudienceRequest],
                    ~.CombinedAudience]:
        ...
        if 'get_combined_audience' not in self._stubs:
            self._stubs['get_combined_audience'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.CombinedAudienceService/GetCombinedAudience',
                request_serializer=combined_audience_service.GetCombinedAudienceRequest.serialize,
                response_deserializer=combined_audience.CombinedAudience.deserialize,
            )
        return self._stubs['get_combined_audience']


__all__ = (
    'CombinedAudienceServiceGrpcTransport',
)
