...
class FeedItemTargetServiceTransport(metaclass=abc.ABCMeta):
    """Abstract transport class for FeedItemTargetService."""
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.get_feed_item_target: gapic_v1.method.wrap_method(
                self.get_feed_item_target,
                default_timeout=None,
                client_info=client_info,
            ),
            self.mutate_feed_item_targets: gapic_v1.method.wrap_method(
                self.mutate_feed_item_targets,
                default_timeout=None,
                client_info=client_info,
            ),
         }
    ...
    @property
    def get_feed_item_target(self) -> typing.Callable[
            [feed_item_target_service.GetFeedItemTargetRequest],
            feed_item_target.FeedItemTarget]:
        raise NotImplementedError

    @property
    def mutate_feed_item_targets(self) -> typing.Callable[
