...
class CustomerUserAccessInvitationServiceGrpcTransport(CustomerUserAccessInvitationServiceTransport):
    """gRPC backend transport for CustomerUserAccessInvitationService.

    This service manages the access invitation extended to users
    ...
    def get_customer_user_access_invitation(self) -> Callable[
        ...
        Returns the requested access invitation in full
        detail.

        Returns:
            Callable[[~.GetCustomerUserAccessInvitationRequest],
        ...
        if 'get_customer_user_access_invitation' not in self._stubs:
            self._stubs['get_customer_user_access_invitation'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.CustomerUserAccessInvitationService/GetCustomerUserAccessInvitation',
                request_serializer=customer_user_access_invitation_service.GetCustomerUserAccessInvitationRequest.serialize,
                response_deserializer=customer_user_access_invitation.CustomerUserAccessInvitation.deserialize,
            )
        return self._stubs['get_customer_user_access_invitation']
    ...
    def mutate_customer_user_access_invitation(self) -> Callable[
        ...
        Returns:
            Callable[[~.MutateCustomerUserAccessInvitationRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_customer_user_access_invitation' not in self._stubs:
            self._stubs['mutate_customer_user_access_invitation'] = self.grpc_channel.unary_unary(
                ...
                response_deserializer=customer_user_access_invitation_service.MutateCustomerUserAccessInvitationResponse.deserialize,
            )
        return self._stubs['mutate_customer_user_access_invitation']
