...
class FeedItemSetLinkServiceGrpcTransport(FeedItemSetLinkServiceTransport):
    """gRPC backend transport for FeedItemSetLinkService.
    ...
    def get_feed_item_set_link(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetFeedItemSetLinkRequest],
                    ~.FeedItemSetLink]:
        ...
        if 'get_feed_item_set_link' not in self._stubs:
            self._stubs['get_feed_item_set_link'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemSetLinkService/GetFeedItemSetLink',
                request_serializer=feed_item_set_link_service.GetFeedItemSetLinkRequest.serialize,
                response_deserializer=feed_item_set_link.FeedItemSetLink.deserialize,
            )
        return self._stubs['get_feed_item_set_link']
    ...
    def mutate_feed_item_set_links(self) -> Callable[
        ...
        Returns:
            Callable[[~.MutateFeedItemSetLinksRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_feed_item_set_links' not in self._stubs:
            self._stubs['mutate_feed_item_set_links'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemSetLinkService/MutateFeedItemSetLinks',
                request_serializer=feed_item_set_link_service.MutateFeedItemSetLinksRequest.serialize,
                response_deserializer=feed_item_set_link_service.MutateFeedItemSetLinksResponse.deserialize,
            )
        return self._stubs['mutate_feed_item_set_links']
