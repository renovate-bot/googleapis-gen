...
class FeedItemTargetServiceGrpcTransport(FeedItemTargetServiceTransport):
    """gRPC backend transport for FeedItemTargetService.
    ...
    def get_feed_item_target(self) -> Callable[
        ...
        Returns the requested feed item targets in full
        detail.

        Returns:
            Callable[[~.GetFeedItemTargetRequest],
                    ~.FeedItemTarget]:
        ...
        if 'get_feed_item_target' not in self._stubs:
            self._stubs['get_feed_item_target'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemTargetService/GetFeedItemTarget',
                request_serializer=feed_item_target_service.GetFeedItemTargetRequest.serialize,
                response_deserializer=feed_item_target.FeedItemTarget.deserialize,
            )
        return self._stubs['get_feed_item_target']
    ...
    def mutate_feed_item_targets(self) -> Callable[
        ...
        Creates or removes feed item targets. Operation
        statuses are returned.

        Returns:
            Callable[[~.MutateFeedItemTargetsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_feed_item_targets' not in self._stubs:
            self._stubs['mutate_feed_item_targets'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemTargetService/MutateFeedItemTargets',
                request_serializer=feed_item_target_service.MutateFeedItemTargetsRequest.serialize,
                response_deserializer=feed_item_target_service.MutateFeedItemTargetsResponse.deserialize,
            )
        return self._stubs['mutate_feed_item_targets']
