...
class CampaignBudgetServiceTransport(metaclass=abc.ABCMeta):
    """Abstract transport class for CampaignBudgetService."""
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.get_campaign_budget: gapic_v1.method.wrap_method(
                self.get_campaign_budget,
                default_timeout=None,
                client_info=client_info,
            ),
            self.mutate_campaign_budgets: gapic_v1.method.wrap_method(
                self.mutate_campaign_budgets,
                default_timeout=None,
                client_info=client_info,
            ),
         }
    ...
    @property
    def get_campaign_budget(self) -> typing.Callable[
            [campaign_budget_service.GetCampaignBudgetRequest],
            campaign_budget.CampaignBudget]:
        raise NotImplementedError

    @property
    def mutate_campaign_budgets(self) -> typing.Callable[
