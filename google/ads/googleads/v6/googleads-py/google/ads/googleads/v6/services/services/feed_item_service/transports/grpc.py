...
class FeedItemServiceGrpcTransport(FeedItemServiceTransport):
    """gRPC backend transport for FeedItemService.
    ...
    def get_feed_item(self) -> Callable[
        ...
        if 'get_feed_item' not in self._stubs:
            self._stubs['get_feed_item'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemService/GetFeedItem',
                request_serializer=feed_item_service.GetFeedItemRequest.serialize,
                response_deserializer=feed_item.FeedItem.deserialize,
            )
        return self._stubs['get_feed_item']
    ...
    def mutate_feed_items(self) -> Callable[
        ...
        Returns:
            Callable[[~.MutateFeedItemsRequest],
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_feed_items' not in self._stubs:
            self._stubs['mutate_feed_items'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v6.services.FeedItemService/MutateFeedItems',
                request_serializer=feed_item_service.MutateFeedItemsRequest.serialize,
                response_deserializer=feed_item_service.MutateFeedItemsResponse.deserialize,
            )
        return self._stubs['mutate_feed_items']
