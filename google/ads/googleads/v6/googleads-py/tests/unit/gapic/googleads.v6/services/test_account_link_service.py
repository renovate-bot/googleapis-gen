...
def test_mutate_account_link_flattened():
    ...
    with mock.patch.object(
        ...
        client.mutate_account_link(
            customer_id='customer_id_value',
            operation=account_link_service.AccountLinkOperation(update_mask=field_mask.FieldMask(paths=['paths_value'])),
        )
...
def test_account_link_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v6.services.services.account_link_service.transports.AccountLinkServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AccountLinkServiceTransport()
        adc.assert_called_once()
