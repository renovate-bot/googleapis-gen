...
def test_account_budget_proposal_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v6.services.services.account_budget_proposal_service.transports.AccountBudgetProposalServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AccountBudgetProposalServiceTransport()
        adc.assert_called_once()
