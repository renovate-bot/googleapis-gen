...
def test_customer_negative_criterion_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v6.services.services.customer_negative_criterion_service.transports.CustomerNegativeCriterionServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.CustomerNegativeCriterionServiceTransport()
        adc.assert_called_once()
