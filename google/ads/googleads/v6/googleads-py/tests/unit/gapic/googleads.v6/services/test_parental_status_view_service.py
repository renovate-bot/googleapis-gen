...
def test_parental_status_view_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v6.services.services.parental_status_view_service.transports.ParentalStatusViewServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.ParentalStatusViewServiceTransport()
        adc.assert_called_once()
