...
def test_billing_setup_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default') as adc, mock.patch('google.ads.googleads.v4.services.services.billing_setup_service.transports.BillingSetupServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.BillingSetupServiceTransport()
        adc.assert_called_once()
