...
def test_get_ad(transport: str = 'grpc', request_type=ad_service.GetAdRequest):
    ...
    with mock.patch.object(
        ...
        call.return_value = ad.Ad(
            ...
            text_ad=ad_type_infos.TextAdInfo(headline=wrappers.StringValue(value='value_value')),
        )
...
def test_ad_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v4.services.services.ad_service.transports.AdServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AdServiceTransport()
        adc.assert_called_once()
