...
def test_get_asset(transport: str = 'grpc', request_type=asset_service.GetAssetRequest):
    ...
    with mock.patch.object(
        ...
        call.return_value = asset.Asset(
            ...
            youtube_video_asset=asset_types.YoutubeVideoAsset(youtube_video_id=wrappers.StringValue(value='value_value')),
        )
...
def test_asset_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.ads.googleads.v4.services.services.asset_service.transports.AssetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AssetServiceTransport()
        adc.assert_called_once()
