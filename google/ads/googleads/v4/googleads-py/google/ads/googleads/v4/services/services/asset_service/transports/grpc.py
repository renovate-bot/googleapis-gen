...
class AssetServiceGrpcTransport(AssetServiceTransport):
    ...
    def get_asset(self) -> Callable[
        ...
        if 'get_asset' not in self._stubs:
            self._stubs['get_asset'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=asset_service.GetAssetRequest.serialize,
                response_deserializer=asset.Asset.deserialize,
            )
    ...
    def mutate_assets(self) -> Callable[
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'mutate_assets' not in self._stubs:
            self._stubs['mutate_assets'] = self.grpc_channel.unary_unary(
                '/google.ads.googleads.v4.services.AssetService/MutateAssets',
                request_serializer=asset_service.MutateAssetsRequest.serialize,
                response_deserializer=asset_service.MutateAssetsResponse.deserialize,
            )
        return self._stubs['mutate_assets']
