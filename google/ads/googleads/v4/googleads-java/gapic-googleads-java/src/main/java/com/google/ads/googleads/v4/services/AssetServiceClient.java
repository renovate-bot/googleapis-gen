...
import com.google.ads.googleads.v4.resources.AssetName;
import com.google.ads.googleads.v4.services.stub.AssetServiceStub;
...
public class AssetServiceClient implements BackgroundResource {
   ...
   * <pre>{@code
   * try (AssetServiceClient assetServiceClient = AssetServiceClient.create()) {
   *   String resourceName = AssetName.of("[CUSTOMER]", "[ASSET]").toString();
   *   Asset response = assetServiceClient.getAsset(resourceName);
   * }
   * }</pre>
   *
   ...
   *   GetAssetRequest request =
   *       GetAssetRequest.newBuilder()
   *           .setResourceName(AssetName.of("[CUSTOMER]", "[ASSET]").toString())
   *           .build();
