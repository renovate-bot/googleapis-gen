...
class AdMobApiGrpcAsyncIOTransport(AdMobApiTransport):
    """gRPC AsyncIO backend transport for AdMobApi.
    ...
    def get_publisher_account(self) -> Callable[
        ...
        Returns:
            Callable[[~.GetPublisherAccountRequest],
                    Awaitable[~.PublisherAccount]]:
        ...
        if 'get_publisher_account' not in self._stubs:
            self._stubs['get_publisher_account'] = self.grpc_channel.unary_unary(
                '/google.ads.admob.v1.AdMobApi/GetPublisherAccount',
                request_serializer=admob_api.GetPublisherAccountRequest.serialize,
                response_deserializer=admob_resources.PublisherAccount.deserialize,
            )
        return self._stubs['get_publisher_account']
    ...
    def list_publisher_accounts(self) -> Callable[
        ...
        Returns:
            Callable[[~.ListPublisherAccountsRequest],
                    Awaitable[~.ListPublisherAccountsResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'list_publisher_accounts' not in self._stubs:
            self._stubs['list_publisher_accounts'] = self.grpc_channel.unary_unary(
                '/google.ads.admob.v1.AdMobApi/ListPublisherAccounts',
                request_serializer=admob_api.ListPublisherAccountsRequest.serialize,
                response_deserializer=admob_api.ListPublisherAccountsResponse.deserialize,
            )
        return self._stubs['list_publisher_accounts']
    ...
    def generate_network_report(self) -> Callable[
        ...
        Returns:
            Callable[[~.GenerateNetworkReportRequest],
                    Awaitable[~.GenerateNetworkReportResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'generate_network_report' not in self._stubs:
            self._stubs['generate_network_report'] = self.grpc_channel.unary_stream(
                '/google.ads.admob.v1.AdMobApi/GenerateNetworkReport',
                request_serializer=admob_api.GenerateNetworkReportRequest.serialize,
                response_deserializer=admob_api.GenerateNetworkReportResponse.deserialize,
            )
        return self._stubs['generate_network_report']
    ...
    def generate_mediation_report(self) -> Callable[
        ...
        Returns:
            Callable[[~.GenerateMediationReportRequest],
                    Awaitable[~.GenerateMediationReportResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'generate_mediation_report' not in self._stubs:
            self._stubs['generate_mediation_report'] = self.grpc_channel.unary_stream(
                '/google.ads.admob.v1.AdMobApi/GenerateMediationReport',
                request_serializer=admob_api.GenerateMediationReportRequest.serialize,
                response_deserializer=admob_api.GenerateMediationReportResponse.deserialize,
            )
