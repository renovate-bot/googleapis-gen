...
def test_trace_service_client_client_options_from_dict():
    with mock.patch('google.devtools.cloudtrace_v1.services.trace_service.transports.TraceServiceGrpcTransport.__init__') as grpc_transport:
...
def test_trace_service_base_transport():
    ...
    with mock.patch('google.devtools.cloudtrace_v1.services.trace_service.transports.TraceServiceTransport.__init__') as Transport:
...
def test_trace_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file') as load_creds, mock.patch('google.devtools.cloudtrace_v1.services.trace_service.transports.TraceServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_trace_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default') as adc, mock.patch('google.devtools.cloudtrace_v1.services.trace_service.transports.TraceServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.TraceServiceTransport()
        adc.assert_called_once()
...
def test_trace_service_transport_auth_adc():
    ...
    with mock.patch.object(auth, 'default') as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transports.TraceServiceGrpcTransport(host="squid.clam.whelk", quota_project_id="octopus")
        adc.assert_called_once_with(scopes=(
            'https://www.googleapis.com/auth/cloud-platform',
            'https://www.googleapis.com/auth/trace.append',
            'https://www.googleapis.com/auth/trace.readonly',),
