 ...
 * `src/v1/publish_build_event_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
import * as gapicConfig from './publish_build_event_client_config.json';
...
export class PublishBuildEventClient {
  ...
  publishBuildEventStub?: Promise<{[name: string]: Function}>;

  /**
   * Construct an instance of PublishBuildEventClient.
   *
  ...
  constructor(opts?: ClientOptions) {
    ...
    const staticMembers = this.constructor as typeof PublishBuildEventClient;
    ...
    this.descriptors.stream = {
      publishBuildToolEventStream: new this._gaxModule.StreamDescriptor(gax.StreamType.BIDI_STREAMING)
    };

    // Put together the default options sent with requests.
    this._defaults = this._gaxGrpc.constructSettings(
        'google.devtools.build.v1.PublishBuildEvent', gapicConfig as gax.ClientConfig,
  ...
  initialize() {
    ...
    if (this.publishBuildEventStub) {
      return this.publishBuildEventStub;
    }

    // Put together the "service stub" for
    // google.devtools.build.v1.PublishBuildEvent.
    this.publishBuildEventStub = this._gaxGrpc.createStub(
        this._opts.fallback ?
          (this._protos as protobuf.Root).lookupService('google.devtools.build.v1.PublishBuildEvent') :
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this._protos as any).google.devtools.build.v1.PublishBuildEvent,
        this._opts) as Promise<{[method: string]: Function}>;

    // Iterate over each of the methods that the service provides
    // and create an API call method for each.
    const publishBuildEventStubMethods =
        ['publishLifecycleEvent', 'publishBuildToolEventStream'];
    for (const methodName of publishBuildEventStubMethods) {
      const callPromise = this.publishBuildEventStub.then(
        stub => (...args: Array<{}>) => {
          if (this._terminated) {
  ...
  static get servicePath() {
    return 'buildeventservice.googleapis.com';
  }
 ...
 * @param {string[]} request.notificationKeywords
 ...
 */
  publishLifecycleEvent(
    ...
    this.initialize();
    return this.innerApiCalls.publishLifecycleEvent(request, options, callback);
  }
 ...
 *   representing [PublishBuildToolEventStreamRequest]{@link google.devtools.build.v1.PublishBuildToolEventStreamRequest} for write() method, and
 *   will emit objects representing [PublishBuildToolEventStreamResponse]{@link google.devtools.build.v1.PublishBuildToolEventStreamResponse} on 'data' event asynchronously.
 *   Please see the
 ...
 */
  publishBuildToolEventStream(
    ...
    this.initialize();
    return this.innerApiCalls.publishBuildToolEventStream(options);
  }
  ...
  close(): Promise<void> {
    this.initialize();
    if (!this._terminated) {
      return this.publishBuildEventStub!.then(stub => {
