...
import * as publishbuildeventModule from '../src';
...
describe('v1.PublishBuildEventClient', () => {
    it('has servicePath', () => {
        const servicePath = publishbuildeventModule.v1.PublishBuildEventClient.servicePath;
        assert(servicePath);
    });

    it('has apiEndpoint', () => {
        const apiEndpoint = publishbuildeventModule.v1.PublishBuildEventClient.apiEndpoint;
        assert(apiEndpoint);
    });

    it('has port', () => {
        const port = publishbuildeventModule.v1.PublishBuildEventClient.port;
        assert(port);
    ...
    it('has initialize method and supports deferred initialization', async () => {
        ...
        assert(client.publishBuildEventStub);
    });
    ...
    describe('publishLifecycleEvent', () => {
        it('invokes publishLifecycleEvent without error', async () => {
            ...
            client.innerApiCalls.publishLifecycleEvent = stubSimpleCall(expectedResponse);
            const [response] = await client.publishLifecycleEvent(request);
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.publishLifecycleEvent as SinonStub)
        ...
        it('invokes publishLifecycleEvent without error using callback', async () => {
            ...
            const expectedResponse = generateSampleMessage(new protos.google.protobuf.Empty());
            client.innerApiCalls.publishLifecycleEvent = stubSimpleCallWithCallback(expectedResponse);
            const promise = new Promise((resolve, reject) => {
        ...
        it('invokes publishLifecycleEvent with error', async () => {
            ...
            client.innerApiCalls.publishLifecycleEvent = stubSimpleCall(undefined, expectedError);
            await assert.rejects(client.publishLifecycleEvent(request), expectedError);
            assert((client.innerApiCalls.publishLifecycleEvent as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    });
    ...
    describe('publishBuildToolEventStream', () => {
        it('invokes publishBuildToolEventStream without error', async () => {
            ...
            client.innerApiCalls.publishBuildToolEventStream = stubBidiStreamingCall(expectedResponse);
            const stream = client.publishBuildToolEventStream();
            const promise = new Promise((resolve, reject) => {
                stream.on('data', (response: protos.google.devtools.build.v1.PublishBuildToolEventStreamResponse) => {
                    resolve(response);
                });
            ...
            const response = await promise;
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.publishBuildToolEventStream as SinonStub)
        ...
        it('invokes publishBuildToolEventStream with error', async () => {
            ...
            client.innerApiCalls.publishBuildToolEventStream = stubBidiStreamingCall(undefined, expectedError);
            const stream = client.publishBuildToolEventStream();
            const promise = new Promise((resolve, reject) => {
                stream.on('data', (response: protos.google.devtools.build.v1.PublishBuildToolEventStreamResponse) => {
                    resolve(response);
