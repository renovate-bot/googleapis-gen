...
class PublishBuildEventGrpcAsyncIOTransport(PublishBuildEventTransport):
    """gRPC AsyncIO backend transport for PublishBuildEvent.
    ...
    def create_channel(cls,
                       host: str = 'buildeventservice.googleapis.com',
    ...
    def publish_lifecycle_event(self) -> Callable[
        ...
        Returns:
            Callable[[~.PublishLifecycleEventRequest],
                    Awaitable[~.Empty]]:
        ...
        if 'publish_lifecycle_event' not in self._stubs:
            self._stubs['publish_lifecycle_event'] = self.grpc_channel.unary_unary(
                '/google.devtools.build.v1.PublishBuildEvent/PublishLifecycleEvent',
                request_serializer=publish_build_event.PublishLifecycleEventRequest.serialize,
                response_deserializer=empty_pb2.Empty.FromString,
            )
        return self._stubs['publish_lifecycle_event']
    ...
    def publish_build_tool_event_stream(self) -> Callable[
        ...
        Returns:
            Callable[[~.PublishBuildToolEventStreamRequest],
                    Awaitable[~.PublishBuildToolEventStreamResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'publish_build_tool_event_stream' not in self._stubs:
            self._stubs['publish_build_tool_event_stream'] = self.grpc_channel.stream_stream(
                '/google.devtools.build.v1.PublishBuildEvent/PublishBuildToolEventStream',
                request_serializer=publish_build_event.PublishBuildToolEventStreamRequest.serialize,
                response_deserializer=publish_build_event.PublishBuildToolEventStreamResponse.deserialize,
            )
