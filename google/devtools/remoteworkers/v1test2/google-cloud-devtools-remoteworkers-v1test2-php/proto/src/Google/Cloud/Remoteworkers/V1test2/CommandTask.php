<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/devtools/remoteworkers/v1test2/command.proto

namespace Google\Cloud\Remoteworkers\V1test2;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Describes a shell-style task to execute, suitable for providing as the Bots
 * interface's `Lease.payload` field.
 *
 * Generated from protobuf message <code>google.devtools.remoteworkers.v1test2.CommandTask</code>
 */
class CommandTask extends \Google\Protobuf\Internal\Message
{
    /**
     * The inputs to the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Inputs inputs = 1;</code>
     */
    protected $inputs = null;
    /**
     * The expected outputs from the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Outputs expected_outputs = 4;</code>
     */
    protected $expected_outputs = null;
    /**
     * The timeouts of this task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Timeouts timeouts = 5;</code>
     */
    protected $timeouts = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type \Google\Cloud\Remoteworkers\V1test2\CommandTask\Inputs $inputs
     *           The inputs to the task.
     *     @type \Google\Cloud\Remoteworkers\V1test2\CommandTask\Outputs $expected_outputs
     *           The expected outputs from the task.
     *     @type \Google\Cloud\Remoteworkers\V1test2\CommandTask\Timeouts $timeouts
     *           The timeouts of this task.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Google\Devtools\Remoteworkers\V1Test2\Command::initOnce();
        parent::__construct($data);
    }

    /**
     * The inputs to the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Inputs inputs = 1;</code>
     * @return \Google\Cloud\Remoteworkers\V1test2\CommandTask\Inputs|null
     */
    public function getInputs()
    {
        return $this->inputs;
    }

    public function hasInputs()
    {
        return isset($this->inputs);
    }

    public function clearInputs()
    {
        unset($this->inputs);
    }

    /**
     * The inputs to the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Inputs inputs = 1;</code>
     * @param \Google\Cloud\Remoteworkers\V1test2\CommandTask\Inputs $var
     * @return $this
     */
    public function setInputs($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\Remoteworkers\V1test2\CommandTask\Inputs::class);
        $this->inputs = $var;

        return $this;
    }

    /**
     * The expected outputs from the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Outputs expected_outputs = 4;</code>
     * @return \Google\Cloud\Remoteworkers\V1test2\CommandTask\Outputs|null
     */
    public function getExpectedOutputs()
    {
        return $this->expected_outputs;
    }

    public function hasExpectedOutputs()
    {
        return isset($this->expected_outputs);
    }

    public function clearExpectedOutputs()
    {
        unset($this->expected_outputs);
    }

    /**
     * The expected outputs from the task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Outputs expected_outputs = 4;</code>
     * @param \Google\Cloud\Remoteworkers\V1test2\CommandTask\Outputs $var
     * @return $this
     */
    public function setExpectedOutputs($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\Remoteworkers\V1test2\CommandTask\Outputs::class);
        $this->expected_outputs = $var;

        return $this;
    }

    /**
     * The timeouts of this task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Timeouts timeouts = 5;</code>
     * @return \Google\Cloud\Remoteworkers\V1test2\CommandTask\Timeouts|null
     */
    public function getTimeouts()
    {
        return $this->timeouts;
    }

    public function hasTimeouts()
    {
        return isset($this->timeouts);
    }

    public function clearTimeouts()
    {
        unset($this->timeouts);
    }

    /**
     * The timeouts of this task.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.CommandTask.Timeouts timeouts = 5;</code>
     * @param \Google\Cloud\Remoteworkers\V1test2\CommandTask\Timeouts $var
     * @return $this
     */
    public function setTimeouts($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\Remoteworkers\V1test2\CommandTask\Timeouts::class);
        $this->timeouts = $var;

        return $this;
    }

}

