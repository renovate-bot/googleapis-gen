<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/devtools/remoteworkers/v1test2/bots.proto

namespace Google\Cloud\Remoteworkers\V1test2;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Request message for CreateBotSession.
 *
 * Generated from protobuf message <code>google.devtools.remoteworkers.v1test2.CreateBotSessionRequest</code>
 */
class CreateBotSessionRequest extends \Google\Protobuf\Internal\Message
{
    /**
     * Required. The farm resource.
     *
     * Generated from protobuf field <code>string parent = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     */
    protected $parent = '';
    /**
     * Required. The bot session to create. Server-assigned fields like name must be unset.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.BotSession bot_session = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     */
    protected $bot_session = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type string $parent
     *           Required. The farm resource.
     *     @type \Google\Cloud\Remoteworkers\V1test2\BotSession $bot_session
     *           Required. The bot session to create. Server-assigned fields like name must be unset.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Google\Devtools\Remoteworkers\V1Test2\Bots::initOnce();
        parent::__construct($data);
    }

    /**
     * Required. The farm resource.
     *
     * Generated from protobuf field <code>string parent = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     * @return string
     */
    public function getParent()
    {
        return $this->parent;
    }

    /**
     * Required. The farm resource.
     *
     * Generated from protobuf field <code>string parent = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     * @param string $var
     * @return $this
     */
    public function setParent($var)
    {
        GPBUtil::checkString($var, True);
        $this->parent = $var;

        return $this;
    }

    /**
     * Required. The bot session to create. Server-assigned fields like name must be unset.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.BotSession bot_session = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     * @return \Google\Cloud\Remoteworkers\V1test2\BotSession|null
     */
    public function getBotSession()
    {
        return $this->bot_session;
    }

    public function hasBotSession()
    {
        return isset($this->bot_session);
    }

    public function clearBotSession()
    {
        unset($this->bot_session);
    }

    /**
     * Required. The bot session to create. Server-assigned fields like name must be unset.
     *
     * Generated from protobuf field <code>.google.devtools.remoteworkers.v1test2.BotSession bot_session = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     * @param \Google\Cloud\Remoteworkers\V1test2\BotSession $var
     * @return $this
     */
    public function setBotSession($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\Remoteworkers\V1test2\BotSession::class);
        $this->bot_session = $var;

        return $this;
    }

}

