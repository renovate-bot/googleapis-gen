...
from google.api.servicecontrol_v1.types import quota_controller
...
class QuotaControllerGrpcTransport(QuotaControllerTransport):
    ...
    def allocate_quota(self) -> Callable[
        ...
        **NOTE:** The client **must** fail-open on server errors
        ...
        if 'allocate_quota' not in self._stubs:
            self._stubs['allocate_quota'] = self.grpc_channel.unary_unary(
                '/google.api.servicecontrol.v1.QuotaController/AllocateQuota',
                request_serializer=quota_controller.AllocateQuotaRequest.serialize,
                response_deserializer=quota_controller.AllocateQuotaResponse.deserialize,
            )
