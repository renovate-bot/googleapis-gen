...
class ServiceControllerGrpcAsyncIOTransport(ServiceControllerTransport):
    ...
    def check(self) -> Callable[
        ...
        if 'check' not in self._stubs:
            self._stubs['check'] = self.grpc_channel.unary_unary(
                '/google.api.servicecontrol.v1.ServiceController/Check',
                request_serializer=service_controller.CheckRequest.serialize,
                response_deserializer=service_controller.CheckResponse.deserialize,
            )
        return self._stubs['check']
    ...
    def report(self) -> Callable[
        ...
        Returns:
            Callable[[~.ReportRequest],
                    Awaitable[~.ReportResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'report' not in self._stubs:
            self._stubs['report'] = self.grpc_channel.unary_unary(
                '/google.api.servicecontrol.v1.ServiceController/Report',
                request_serializer=service_controller.ReportRequest.serialize,
                response_deserializer=service_controller.ReportResponse.deserialize,
            )
        return self._stubs['report']


__all__ = (
    'ServiceControllerGrpcAsyncIOTransport',
)
