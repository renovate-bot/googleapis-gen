...
class CloudTasksClient(metaclass=CloudTasksClientMeta):
    ...
    def queue_path(project: str,location: str,queue: str,) -> str:
        ...
        return "projects/{project}/locations/{location}/queues/{queue}".format(project=project, location=location, queue=queue, )

    @staticmethod
    def parse_queue_path(path: str) -> Dict[str,str]:
        """Parse a queue path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/queues/(?P<queue>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def task_path(project: str,location: str,queue: str,task: str,) -> str:
        """Return a fully-qualified task string."""
        return "projects/{project}/locations/{location}/queues/{queue}/tasks/{task}".format(project=project, location=location, queue=queue, task=task, )

    @staticmethod
    def parse_task_path(path: str) -> Dict[str,str]:
        """Parse a task path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/queues/(?P<queue>.+?)/tasks/(?P<task>.+?)$", path)
        return m.groupdict() if m else {}
