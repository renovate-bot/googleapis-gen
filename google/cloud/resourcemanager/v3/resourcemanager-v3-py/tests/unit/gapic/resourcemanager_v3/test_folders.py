...
def test_folders_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_create_folder(transport: str = 'grpc', request_type=folders.CreateFolderRequest):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.create_folder),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_folders_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.FoldersGrpcTransport, transports.FoldersGrpcAsyncIOTransport])
def test_folders_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.FoldersGrpcTransport, transports.FoldersGrpcAsyncIOTransport])
def test_folders_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
