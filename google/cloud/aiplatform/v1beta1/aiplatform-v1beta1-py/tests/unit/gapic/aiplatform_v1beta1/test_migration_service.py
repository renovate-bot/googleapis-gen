...
from google.cloud.aiplatform_v1beta1.services.migration_service.transports.base import _API_CORE_VERSION
from google.cloud.aiplatform_v1beta1.services.migration_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_batch_migrate_resources(transport: str = 'grpc', request_type=migration_service.BatchMigrateResourcesRequest):
    client = MigrationServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_batch_migrate_resources_flattened():
    ...
    with mock.patch.object(
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == 'parent_value'
        assert args[0].migrate_resource_requests == [migration_service.MigrateResourceRequest(migrate_ml_engine_model_version_config=migration_service.MigrateResourceRequest.MigrateMlEngineModelVersionConfig(endpoint='endpoint_value'))]
...
def test_migration_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.aiplatform_v1beta1.services.migration_service.transports.MigrationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_migration_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.aiplatform_v1beta1.services.migration_service.transports.MigrationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.MigrationServiceTransport()
        adc.assert_called_once()
