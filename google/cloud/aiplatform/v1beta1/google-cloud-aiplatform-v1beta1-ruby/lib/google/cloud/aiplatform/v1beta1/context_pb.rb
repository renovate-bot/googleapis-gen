# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/cloud/aiplatform/v1beta1/context.proto

require 'google/protobuf'

require 'google/api/field_behavior_pb'
require 'google/api/resource_pb'
require 'google/cloud/aiplatform/v1beta1/value_pb'
require 'google/protobuf/struct_pb'
require 'google/protobuf/timestamp_pb'
require 'google/api/annotations_pb'
Google::Protobuf::DescriptorPool.generated_pool.build do
  add_file("google/cloud/aiplatform/v1beta1/context.proto", :syntax => :proto3) do
    add_message "google.cloud.aiplatform.v1beta1.Context" do
      optional :name, :string, 1
      optional :display_name, :string, 2
      optional :etag, :string, 8
      map :labels, :string, :string, 9
      optional :create_time, :message, 10, "google.protobuf.Timestamp"
      optional :update_time, :message, 11, "google.protobuf.Timestamp"
      repeated :parent_contexts, :string, 12
      optional :schema_title, :string, 13
      optional :schema_version, :string, 14
      optional :metadata, :message, 15, "google.protobuf.Struct"
      optional :description, :string, 16
    end
  end
end

module Google
  module Cloud
    module AIPlatform
      module V1beta1
        Context = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.cloud.aiplatform.v1beta1.Context").msgclass
      end
    end
  end
end
