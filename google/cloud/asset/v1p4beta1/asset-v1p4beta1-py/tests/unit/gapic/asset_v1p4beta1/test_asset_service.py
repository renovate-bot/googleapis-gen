...
from google.cloud.asset_v1p4beta1.services.asset_service.transports.base import _API_CORE_VERSION
from google.cloud.asset_v1p4beta1.services.asset_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_asset_service_client_client_options_from_dict():
    with mock.patch('google.cloud.asset_v1p4beta1.services.asset_service.transports.AssetServiceGrpcTransport.__init__') as grpc_transport:
...
def test_export_iam_policy_analysis(transport: str = 'grpc', request_type=asset_service.ExportIamPolicyAnalysisRequest):
    client = AssetServiceClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.export_iam_policy_analysis),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_export_iam_policy_analysis_async(transport: str = 'grpc_asyncio', request_type=asset_service.ExportIamPolicyAnalysisRequest):
    client = AssetServiceAsyncClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_export_iam_policy_analysis_async_from_dict():
    await test_export_iam_policy_analysis_async(request_type=dict)
...
def test_asset_service_base_transport():
    ...
    with mock.patch('google.cloud.asset_v1p4beta1.services.asset_service.transports.AssetServiceTransport.__init__') as Transport:
...
def test_asset_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.asset_v1p4beta1.services.asset_service.transports.AssetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_asset_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.asset_v1p4beta1.services.asset_service.transports.AssetServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AssetServiceTransport()
        adc.assert_called_once()
