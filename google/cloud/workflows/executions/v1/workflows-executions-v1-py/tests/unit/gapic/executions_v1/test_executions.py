...
from google.cloud.workflows.executions_v1.services.executions import ExecutionsAsyncClient
...
from google.cloud.workflows.executions_v1.services.executions.transports.base import _API_CORE_VERSION
from google.cloud.workflows.executions_v1.services.executions.transports.base import _GOOGLE_AUTH_VERSION
...
def test_executions_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_executions_base_transport():
    ...
    with mock.patch('google.cloud.workflows.executions_v1.services.executions.transports.ExecutionsTransport.__init__') as Transport:
...
def test_executions_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.workflows.executions_v1.services.executions.transports.ExecutionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_executions_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.workflows.executions_v1.services.executions.transports.ExecutionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ExecutionsTransport()
        adc.assert_called_once()
...
def test_executions_transport_create_channel_old_api_core(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(auth, "default", autospec=True) as adc, mock.patch.object(
        grpc_helpers, "create_channel", autospec=True
...
def test_execution_path():
    ...
    workflow = "whelk"
    execution = "octopus"
    expected = "projects/{project}/locations/{location}/workflows/{workflow}/executions/{execution}".format(project=project, location=location, workflow=workflow, execution=execution, )
