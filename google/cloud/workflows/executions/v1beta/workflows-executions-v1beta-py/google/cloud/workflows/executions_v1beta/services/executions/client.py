...
class ExecutionsClient(metaclass=ExecutionsClientMeta):
    ...
    @staticmethod
    def execution_path(project: str,location: str,workflow: str,execution: str,) -> str:
        """Return a fully-qualified execution string."""
        return "projects/{project}/locations/{location}/workflows/{workflow}/executions/{execution}".format(project=project, location=location, workflow=workflow, execution=execution, )

    @staticmethod
    def parse_execution_path(path: str) -> Dict[str,str]:
        """Parse a execution path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/workflows/(?P<workflow>.+?)/executions/(?P<execution>.+?)$", path)
