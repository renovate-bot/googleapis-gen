...
class EnvironmentsClient(metaclass=EnvironmentsClientMeta):
    ...
    def environment_path(project: str,location: str,environment: str,) -> str:
        ...
        return "projects/{project}/locations/{location}/environments/{environment}".format(project=project, location=location, environment=environment, )

    @staticmethod
    def parse_environment_path(path: str) -> Dict[str,str]:
        """Parses a environment path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/environments/(?P<environment>.+?)$", path)
