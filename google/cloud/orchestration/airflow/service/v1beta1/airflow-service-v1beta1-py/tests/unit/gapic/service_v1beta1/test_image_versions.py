...
from google.cloud.orchestration.airflow.service_v1beta1.services.image_versions.transports.base import _GOOGLE_AUTH_VERSION
...
def test_image_versions_client_service_account_always_use_jwt(client_class):
...
def test_image_versions_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_list_image_versions(transport: str = 'grpc', request_type=image_versions.ListImageVersionsRequest):
    client = ImageVersionsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_image_versions_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.orchestration.airflow.service_v1beta1.services.image_versions.transports.ImageVersionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@requires_google_auth_lt_1_25_0
def test_image_versions_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        ImageVersionsClient()
...
@requires_google_auth_lt_1_25_0
def test_image_versions_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
