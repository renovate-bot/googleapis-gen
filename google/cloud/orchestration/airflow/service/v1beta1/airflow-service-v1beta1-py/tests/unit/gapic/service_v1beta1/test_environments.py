...
from google.cloud.orchestration.airflow.service_v1beta1.services.environments.transports.base import _GOOGLE_AUTH_VERSION
...
def test_create_environment_flattened():
    ...
    with mock.patch.object(
        ...
        assert args[0].environment == environments.Environment(name='name_value')
...
def test_restart_web_server(transport: str = 'grpc', request_type=environments.RestartWebServerRequest):
    client = EnvironmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_restart_web_server_from_dict():
    test_restart_web_server(request_type=dict)
...
def test_check_upgrade(transport: str = 'grpc', request_type=environments.CheckUpgradeRequest):
    client = EnvironmentsClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_check_upgrade_from_dict():
    test_check_upgrade(request_type=dict)
