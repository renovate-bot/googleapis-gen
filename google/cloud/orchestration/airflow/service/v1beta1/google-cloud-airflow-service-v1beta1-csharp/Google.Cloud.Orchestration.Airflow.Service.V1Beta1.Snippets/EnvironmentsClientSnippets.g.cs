...
namespace Google.Cloud.Orchestration.Airflow.Service.V1Beta1.Snippets
{
    ...
    public sealed class GeneratedEnvironmentsClientSnippets
    {
        ...
        {
            // Snippet: RestartWebServer(RestartWebServerRequest, CallSettings)
            // Create client
            EnvironmentsClient environmentsClient = EnvironmentsClient.Create();
            // Initialize request argument(s)
            RestartWebServerRequest request = new RestartWebServerRequest { Name = "", };
            // Make the request
            Operation<gcoasv::Environment, OperationMetadata> response = environmentsClient.RestartWebServer(request);
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<gcoasv::Environment, OperationMetadata> retrievedResponse = environmentsClient.PollOnceRestartWebServer(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
        ...
        {
            // Snippet: RestartWebServerAsync(RestartWebServerRequest, CallSettings)
            // Additional: RestartWebServerAsync(RestartWebServerRequest, CancellationToken)
            // Create client
            EnvironmentsClient environmentsClient = await EnvironmentsClient.CreateAsync();
            // Initialize request argument(s)
            RestartWebServerRequest request = new RestartWebServerRequest { Name = "", };
            // Make the request
            Operation<gcoasv::Environment, OperationMetadata> response = await environmentsClient.RestartWebServerAsync(request);
            ...
            string operationName = response.Name;
            // This name can be stored, then the long-running operation retrieved later by name
            Operation<gcoasv::Environment, OperationMetadata> retrievedResponse = await environmentsClient.PollOnceRestartWebServerAsync(operationName);
            // Check if the retrieved long-running operation has completed
            if (retrievedResponse.IsCompleted)
            {
                // If it has completed, then access the result
