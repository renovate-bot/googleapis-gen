...
from google.cloud.vision_v1.services.image_annotator.transports.base import _API_CORE_VERSION
from google.cloud.vision_v1.services.image_annotator.transports.base import _GOOGLE_AUTH_VERSION
...
def test_image_annotator_client_client_options_from_dict():
    with mock.patch('google.cloud.vision_v1.services.image_annotator.transports.ImageAnnotatorGrpcTransport.__init__') as grpc_transport:
...
def test_batch_annotate_files_flattened():
    ...
    with mock.patch.object(
        ...
        # using the keyword arguments to the method.
        client.batch_annotate_files(
            requests=[image_annotator.AnnotateFileRequest(input_config=image_annotator.InputConfig(gcs_source=image_annotator.GcsSource(uri='uri_value')))],
        )
...
def test_async_batch_annotate_images(transport: str = 'grpc', request_type=image_annotator.AsyncBatchAnnotateImagesRequest):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.async_batch_annotate_images),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_async_batch_annotate_images_flattened():
    ...
    with mock.patch.object(
        ...
        client.async_batch_annotate_images(
            requests=[image_annotator.AnnotateImageRequest(image=image_annotator.Image(content=b'content_blob'))],
            output_config=image_annotator.OutputConfig(gcs_destination=image_annotator.GcsDestination(uri='uri_value')),
        )
...
def test_image_annotator_base_transport():
    ...
    with mock.patch('google.cloud.vision_v1.services.image_annotator.transports.ImageAnnotatorTransport.__init__') as Transport:
...
def test_image_annotator_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.vision_v1.services.image_annotator.transports.ImageAnnotatorTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_image_annotator_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.vision_v1.services.image_annotator.transports.ImageAnnotatorTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ImageAnnotatorTransport()
        adc.assert_called_once()
