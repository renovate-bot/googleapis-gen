...
from google.cloud.vision_v1p1beta1.services.image_annotator import ImageAnnotatorAsyncClient
from google.cloud.vision_v1p1beta1.services.image_annotator import ImageAnnotatorClient
from google.cloud.vision_v1p1beta1.services.image_annotator import transports
from google.cloud.vision_v1p1beta1.services.image_annotator.transports.base import _API_CORE_VERSION
from google.cloud.vision_v1p1beta1.services.image_annotator.transports.base import _GOOGLE_AUTH_VERSION
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (ImageAnnotatorAsyncClient, transports.ImageAnnotatorGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_image_annotator_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_image_annotator_client_client_options_from_dict():
    with mock.patch('google.cloud.vision_v1p1beta1.services.image_annotator.transports.ImageAnnotatorGrpcTransport.__init__') as grpc_transport:
...
def test_batch_annotate_images_flattened():
    ...
    with mock.patch.object(
        ...
        client.batch_annotate_images(
            requests=[image_annotator.AnnotateImageRequest(image=image_annotator.Image(content=b'content_blob'))],
        )
...
def test_image_annotator_base_transport():
    ...
    with mock.patch('google.cloud.vision_v1p1beta1.services.image_annotator.transports.ImageAnnotatorTransport.__init__') as Transport:
...
def test_image_annotator_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.vision_v1p1beta1.services.image_annotator.transports.ImageAnnotatorTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_image_annotator_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.vision_v1p1beta1.services.image_annotator.transports.ImageAnnotatorTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ImageAnnotatorTransport()
        adc.assert_called_once()
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ImageAnnotatorGrpcTransport, transports.ImageAnnotatorGrpcAsyncIOTransport])
def test_image_annotator_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
