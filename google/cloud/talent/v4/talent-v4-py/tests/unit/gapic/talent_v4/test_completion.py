...
def test__get_default_mtls_endpoint():
    ...
    assert CompletionClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert CompletionClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert CompletionClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert CompletionClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert CompletionClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test_completion_client_from_service_account_info():
    creds = credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        factory.return_value = creds
        info = {"valid": True}
        client = CompletionClient.from_service_account_info(info)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (CompletionAsyncClient, transports.CompletionGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_completion_client_client_options(client_class, transport_class, transport_name):
...
def test_completion_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_completion_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_completion_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_completion_client_client_options_from_dict():
    with mock.patch('google.cloud.talent_v4.services.completion.transports.CompletionGrpcTransport.__init__') as grpc_transport:
...
def test_completion_base_transport():
    ...
    with mock.patch('google.cloud.talent_v4.services.completion.transports.CompletionTransport.__init__') as Transport:
...
@pytest.mark.parametrize("transport_class", [transports.CompletionGrpcTransport, transports.CompletionGrpcAsyncIOTransport])
def test_completion_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.CompletionGrpcTransport, transports.CompletionGrpcAsyncIOTransport])
def test_completion_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
def test_client_withDEFAULT_CLIENT_INFO():
    ...
    with mock.patch.object(transports.CompletionTransport, '_prep_wrapped_messages') as prep:
        client = CompletionClient(
            credentials=credentials.AnonymousCredentials(),
