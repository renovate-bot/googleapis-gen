...
def test__get_default_mtls_endpoint():
    ...
    assert ProfileServiceClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert ProfileServiceClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert ProfileServiceClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert ProfileServiceClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (ProfileServiceAsyncClient, transports.ProfileServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_profile_service_client_client_options(client_class, transport_class, transport_name):
...
def test_profile_service_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_profile_service_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_profile_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_profile_service_client_client_options_from_dict():
    with mock.patch('google.cloud.talent_v4beta1.services.profile_service.transports.ProfileServiceGrpcTransport.__init__') as grpc_transport:
...
def test_list_profiles_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, profile.Profile)
                   for i in results)
...
def test_search_profiles_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in pager]
        assert len(results) == 6
        assert all(isinstance(i, histogram.HistogramQueryResult)
                   for i in results)
...
def test_profile_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.talent_v4beta1.services.profile_service.transports.ProfileServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_profile_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.talent_v4beta1.services.profile_service.transports.ProfileServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ProfileServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.ProfileServiceGrpcTransport, transports.ProfileServiceGrpcAsyncIOTransport])
