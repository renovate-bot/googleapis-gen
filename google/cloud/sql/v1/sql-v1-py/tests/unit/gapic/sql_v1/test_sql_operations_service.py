...
from google.cloud.sql_v1.services.sql_operations_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_get(transport: str = 'grpc', request_type=cloud_sql_operations.SqlOperationsGetRequest):
    client = SqlOperationsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list(transport: str = 'grpc', request_type=cloud_sql_operations.SqlOperationsListRequest):
    client = SqlOperationsServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_sql_operations_service_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.sql_v1.services.sql_operations_service.transports.SqlOperationsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@requires_google_auth_lt_1_25_0
def test_sql_operations_service_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        SqlOperationsServiceClient()
...
@requires_google_auth_lt_1_25_0
def test_sql_operations_service_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
