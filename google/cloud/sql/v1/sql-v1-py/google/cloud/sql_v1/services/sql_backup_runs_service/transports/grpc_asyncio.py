...
from .base import SqlBackupRunsServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import SqlBackupRunsServiceGrpcTransport


class SqlBackupRunsServiceGrpcAsyncIOTransport(SqlBackupRunsServiceTransport):
    """gRPC AsyncIO backend transport for SqlBackupRunsService.
    ...
    def delete(self) -> Callable[
        ...
        Returns:
            Callable[[~.SqlBackupRunsDeleteRequest],
                    Awaitable[~.Operation]]:
        ...
        if 'delete' not in self._stubs:
            self._stubs['delete'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_backup_runs.SqlBackupRunsDeleteRequest.serialize,
    ...
    def get(self) -> Callable[
            ...
            Awaitable[cloud_sql_backup_runs.BackupRun]]:
        ...
        Returns:
            Callable[[~.SqlBackupRunsGetRequest],
                    Awaitable[~.BackupRun]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'get' not in self._stubs:
            self._stubs['get'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_backup_runs.SqlBackupRunsGetRequest.serialize,
                response_deserializer=cloud_sql_backup_runs.BackupRun.deserialize,
            )
    ...
    def insert(self) -> Callable[
        ...
        if 'insert' not in self._stubs:
            self._stubs['insert'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_backup_runs.SqlBackupRunsInsertRequest.serialize,
                response_deserializer=cloud_sql_resources.Operation.deserialize,
            )
    ...
    def list(self) -> Callable[
            ...
            Awaitable[cloud_sql_backup_runs.BackupRunsListResponse]]:
        ...
        Returns:
            Callable[[~.SqlBackupRunsListRequest],
                    Awaitable[~.BackupRunsListResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        if 'list' not in self._stubs:
            self._stubs['list'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_backup_runs.SqlBackupRunsListRequest.serialize,
                response_deserializer=cloud_sql_backup_runs.BackupRunsListResponse.deserialize,
            )
