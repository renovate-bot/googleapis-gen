...
from .base import SqlSslCertsServiceTransport, DEFAULT_CLIENT_INFO


class SqlSslCertsServiceGrpcTransport(SqlSslCertsServiceTransport):
    """gRPC backend transport for SqlSslCertsService.
    ...
    def delete(self) -> Callable[
        ...
        if 'delete' not in self._stubs:
            self._stubs['delete'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_ssl_certs.SqlSslCertsDeleteRequest.serialize,
    ...
    def get(self) -> Callable[
        ...
        if 'get' not in self._stubs:
            self._stubs['get'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_ssl_certs.SqlSslCertsGetRequest.serialize,
    ...
    def insert(self) -> Callable[
        ...
        if 'insert' not in self._stubs:
            self._stubs['insert'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_ssl_certs.SqlSslCertsInsertRequest.serialize,
                response_deserializer=cloud_sql_ssl_certs.SslCertsInsertResponse.deserialize,
            )
        return self._stubs['insert']

    @property
    def list(self) -> Callable[
            [cloud_sql_ssl_certs.SqlSslCertsListRequest],
            cloud_sql_ssl_certs.SslCertsListResponse]:
        r"""Return a callable for the list method over gRPC.

        Lists all of the current SSL certificates for the
        instance.

        Returns:
            Callable[[~.SqlSslCertsListRequest],
        ...
        if 'list' not in self._stubs:
            self._stubs['list'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_ssl_certs.SqlSslCertsListRequest.serialize,
                response_deserializer=cloud_sql_ssl_certs.SslCertsListResponse.deserialize,
            )
