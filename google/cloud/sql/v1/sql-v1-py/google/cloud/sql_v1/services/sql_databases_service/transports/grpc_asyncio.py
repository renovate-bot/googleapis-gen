...
from .base import SqlDatabasesServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import SqlDatabasesServiceGrpcTransport


class SqlDatabasesServiceGrpcAsyncIOTransport(SqlDatabasesServiceTransport):
    """gRPC AsyncIO backend transport for SqlDatabasesService.
    ...
    def delete(self) -> Callable[
            [cloud_sql_databases.SqlDatabasesDeleteRequest],
        ...
        Returns:
            Callable[[~.SqlDatabasesDeleteRequest],
                    Awaitable[~.Operation]]:
        ...
        if 'delete' not in self._stubs:
            self._stubs['delete'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_databases.SqlDatabasesDeleteRequest.serialize,
    ...
    def get(self) -> Callable[
            [cloud_sql_databases.SqlDatabasesGetRequest],
            Awaitable[cloud_sql_resources.Database]]:
        ...
        Returns:
            Callable[[~.SqlDatabasesGetRequest],
                    Awaitable[~.Database]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'get' not in self._stubs:
            self._stubs['get'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_databases.SqlDatabasesGetRequest.serialize,
                response_deserializer=cloud_sql_resources.Database.deserialize,
            )
        return self._stubs['get']

    @property
    def insert(self) -> Callable[
            [cloud_sql_databases.SqlDatabasesInsertRequest],
            Awaitable[cloud_sql_resources.Operation]]:
        r"""Return a callable for the insert method over gRPC.

        Inserts a resource containing information about a
        database inside a Cloud SQL instance.

        Returns:
            Callable[[~.SqlDatabasesInsertRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'insert' not in self._stubs:
            self._stubs['insert'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_databases.SqlDatabasesInsertRequest.serialize,
                response_deserializer=cloud_sql_resources.Operation.deserialize,
            )
    ...
    def list(self) -> Callable[
            [cloud_sql_databases.SqlDatabasesListRequest],
            Awaitable[cloud_sql_databases.DatabasesListResponse]]:
        r"""Return a callable for the list method over gRPC.

        Lists databases in the specified Cloud SQL instance.

        Returns:
            Callable[[~.SqlDatabasesListRequest],
                    Awaitable[~.DatabasesListResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        if 'list' not in self._stubs:
            self._stubs['list'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_databases.SqlDatabasesListRequest.serialize,
                response_deserializer=cloud_sql_databases.DatabasesListResponse.deserialize,
            )
        return self._stubs['list']

    @property
    def patch(self) -> Callable[
            [cloud_sql_databases.SqlDatabasesUpdateRequest],
            Awaitable[cloud_sql_resources.Operation]]:
        r"""Return a callable for the patch method over gRPC.

        Partially updates a resource containing information
        about a database inside a Cloud SQL instance. This
        method supports patch semantics.

        Returns:
            Callable[[~.SqlDatabasesUpdateRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'patch' not in self._stubs:
            self._stubs['patch'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_databases.SqlDatabasesUpdateRequest.serialize,
                response_deserializer=cloud_sql_resources.Operation.deserialize,
            )
    ...
    def update(self) -> Callable[
        ...
        Updates a resource containing information about a
        database inside a Cloud SQL instance.
