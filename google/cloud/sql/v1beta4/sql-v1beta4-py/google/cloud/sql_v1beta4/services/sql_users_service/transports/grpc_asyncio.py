...
from .base import SqlUsersServiceTransport, DEFAULT_CLIENT_INFO
from .grpc import SqlUsersServiceGrpcTransport


class SqlUsersServiceGrpcAsyncIOTransport(SqlUsersServiceTransport):
    """gRPC AsyncIO backend transport for SqlUsersService.
    ...
    def delete(self) -> Callable[
        ...
        Returns:
            Callable[[~.SqlUsersDeleteRequest],
                    Awaitable[~.Operation]]:
        ...
        if 'delete' not in self._stubs:
            self._stubs['delete'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_users.SqlUsersDeleteRequest.serialize,
    ...
    def insert(self) -> Callable[
        ...
        Returns:
            Callable[[~.SqlUsersInsertRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'insert' not in self._stubs:
            self._stubs['insert'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_users.SqlUsersInsertRequest.serialize,
                response_deserializer=cloud_sql_resources.Operation.deserialize,
            )
    ...
    def list(self) -> Callable[
            ...
            Awaitable[cloud_sql_users.UsersListResponse]]:
        r"""Return a callable for the list method over gRPC.

        Lists users in the specified Cloud SQL instance.

        Returns:
            Callable[[~.SqlUsersListRequest],
                    Awaitable[~.UsersListResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        if 'list' not in self._stubs:
            self._stubs['list'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_users.SqlUsersListRequest.serialize,
                response_deserializer=cloud_sql_users.UsersListResponse.deserialize,
            )
    ...
    def update(self) -> Callable[
        ...
        Returns:
            Callable[[~.SqlUsersUpdateRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        if 'update' not in self._stubs:
            self._stubs['update'] = self.grpc_channel.unary_unary(
                ...
                request_serializer=cloud_sql_users.SqlUsersUpdateRequest.serialize,
                response_deserializer=cloud_sql_resources.Operation.deserialize,
            )
...
__all__ = (
    'SqlUsersServiceGrpcAsyncIOTransport',
)
