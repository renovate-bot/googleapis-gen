...
namespace Google.Cloud.NetworkConnectivity.V1.Snippets
{
    ...
    public sealed partial class GeneratedHubServiceClientStandaloneSnippets
    {
        ...
        /// </remarks>
        public void ActivateSpokeRequestObject()
        {
            // Create client
            HubServiceClient hubServiceClient = HubServiceClient.Create();
            // Initialize request argument(s)
            ActivateSpokeRequest request = new ActivateSpokeRequest
            {
                SpokeName = SpokeName.FromProjectLocationSpoke("[PROJECT]", "[LOCATION]", "[SPOKE]"),
                RequestId = "",
            };
            // Make the request
            Operation<Spoke, OperationMetadata> response = hubServiceClient.ActivateSpoke(request);
