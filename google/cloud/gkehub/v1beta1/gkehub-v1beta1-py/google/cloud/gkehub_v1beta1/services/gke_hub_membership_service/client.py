...
class GkeHubMembershipServiceClientMeta(type):
    ...
    objects.
    """
    _transport_registry = OrderedDict()  # type: Dict[str, Type[GkeHubMembershipServiceTransport]]
...
class GkeHubMembershipServiceClient(metaclass=GkeHubMembershipServiceClientMeta):
    ...
    def membership_path(project: str,location: str,membership: str,) -> str:
        """Return a fully-qualified membership string."""
        return "projects/{project}/locations/{location}/memberships/{membership}".format(project=project, location=location, membership=membership, )

    @staticmethod
    def parse_membership_path(path: str) -> Dict[str,str]:
        """Parse a membership path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/memberships/(?P<membership>.+?)$", path)
    ...
    def generate_connect_manifest(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.generate_connect_manifest]
    ...
    def generate_exclusivity_manifest(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.generate_exclusivity_manifest]
