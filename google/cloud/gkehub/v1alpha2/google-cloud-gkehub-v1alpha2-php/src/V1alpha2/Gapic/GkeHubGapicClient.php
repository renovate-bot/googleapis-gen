 ...
 * https://github.com/googleapis/googleapis/blob/master/google/cloud/gkehub/v1alpha2/membership.proto
 * Updates to the above are reflected here through a refresh process.
 *
 * @experimental
 */

namespace Google\Cloud\GkeHub\V1alpha2\Gapic;
...
use Google\Cloud\GkeHub\V1alpha2\CreateMembershipRequest;
use Google\Cloud\GkeHub\V1alpha2\DeleteMembershipRequest;
use Google\Cloud\GkeHub\V1alpha2\GenerateConnectManifestRequest;
use Google\Cloud\GkeHub\V1alpha2\GenerateConnectManifestResponse;
use Google\Cloud\GkeHub\V1alpha2\GetMembershipRequest;
use Google\Cloud\GkeHub\V1alpha2\InitializeHubRequest;
use Google\Cloud\GkeHub\V1alpha2\InitializeHubResponse;
use Google\Cloud\GkeHub\V1alpha2\ListMembershipsRequest;
use Google\Cloud\GkeHub\V1alpha2\ListMembershipsResponse;
use Google\Cloud\GkeHub\V1alpha2\Membership;
use Google\Cloud\GkeHub\V1alpha2\UpdateMembershipRequest;
 ...
 *     $operationResponse = $gkeHubClient->createMembership($formattedParent, $membershipId, $resource);
 ...
 *     // ... do other work
 *     $newOperationResponse = $gkeHubClient->resumeOperation($operationName, 'createMembership');
...
class GkeHubGapicClient
{
     ...
     */
    const SERVICE_NAME = 'google.cloud.gkehub.v1alpha2.GkeHub';
     ...
     *     $operationResponse = $gkeHubClient->deleteMembership($formattedName);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $gkeHubClient->resumeOperation($operationName, 'deleteMembership');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
     ...
     * try {
     *     $name = 'name';
     *     $response = $gkeHubClient->generateConnectManifest($name);
     * } finally {
     *     $gkeHubClient->close();
     * }
     * ```
    ...
    public function generateConnectManifest($name, array $optionalArgs = [])
    {
        ...
        if (isset($optionalArgs['proxy'])) {
            $request->setProxy($optionalArgs['proxy']);
        }
     ...
     * try {
     *     $formattedName = $gkeHubClient->membershipName('[PROJECT]', '[LOCATION]', '[MEMBERSHIP]');
     *     $response = $gkeHubClient->getMembership($formattedName);
     * } finally {
     *     $gkeHubClient->close();
     * }
     * ```
     ...
     * try {
     *     $project = 'project';
     *     $response = $gkeHubClient->initializeHub($project);
     * } finally {
     *     $gkeHubClient->close();
     * }
     * ```
     *
     * @param string $project      Required. The Hub to initialize, in the format
     *                             `projects/&#42;/locations/&#42;/memberships/*`.
     * @param array  $optionalArgs {
     *     Optional.
     *
     ...
     * @experimental
     */
    public function initializeHub($project, array $optionalArgs = [])
    {
        $request = new InitializeHubRequest();
        $requestParamHeaders = [];
        $request->setProject($project);
        $requestParamHeaders['project'] = $project;
        $requestParams = new RequestParamsHeaderDescriptor($requestParamHeaders);
        $optionalArgs['headers'] = isset($optionalArgs['headers']) ? array_merge($requestParams->getHeader(), $optionalArgs['headers']) : $requestParams->getHeader();
        return $this->startCall('InitializeHub', InitializeHubResponse::class, $optionalArgs, $request)->wait();
    }
     ...
     *     $pagedResponse = $gkeHubClient->listMemberships($formattedParent);
     ...
     *     $resource = new Membership();
     *     $operationResponse = $gkeHubClient->updateMembership($name, $updateMask, $resource);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
     *         $result = $operationResponse->getResult();
     *     // doSomethingWith($result)
     ...
     *     $operationName = $operationResponse->getName();
     *     // ... do other work
     *     $newOperationResponse = $gkeHubClient->resumeOperation($operationName, 'updateMembership');
     *     while (!$newOperationResponse->isDone()) {
     *         // ... do other work
     *         $newOperationResponse->reload();
     *     }
