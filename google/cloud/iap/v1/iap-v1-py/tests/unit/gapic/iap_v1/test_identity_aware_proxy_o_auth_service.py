...
from google.cloud.iap_v1.services.identity_aware_proxy_o_auth_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_identity_aware_proxy_o_auth_service_client_service_account_always_use_jwt(client_class):
...
def test_identity_aware_proxy_o_auth_service_client_service_account_always_use_jwt_true(transport_class, transport_name):
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
...
def test_identity_aware_proxy_o_auth_service_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.iap_v1.services.identity_aware_proxy_o_auth_service.transports.IdentityAwareProxyOAuthServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@requires_google_auth_lt_1_25_0
def test_identity_aware_proxy_o_auth_service_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
