...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (AssuredWorkloadsServiceClient, transports.AssuredWorkloadsServiceGrpcTransport, "grpc"),
    (AssuredWorkloadsServiceAsyncClient, transports.AssuredWorkloadsServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_assured_workloads_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_assured_workloads_service_client_client_options_from_dict():
    with mock.patch('google.cloud.assuredworkloads_v1beta1.services.assured_workloads_service.transports.AssuredWorkloadsServiceGrpcTransport.__init__') as grpc_transport:
...
def test_list_workloads_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, assuredworkloads_v1beta1.Workload)
                   for i in results)
...
def test_assured_workloads_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.assuredworkloads_v1beta1.services.assured_workloads_service.transports.AssuredWorkloadsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_assured_workloads_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.assuredworkloads_v1beta1.services.assured_workloads_service.transports.AssuredWorkloadsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.AssuredWorkloadsServiceTransport()
        adc.assert_called_once()
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.AssuredWorkloadsServiceGrpcTransport, transports.AssuredWorkloadsServiceGrpcAsyncIOTransport])
def test_assured_workloads_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
