...
class PolicyTagManagerClient(metaclass=PolicyTagManagerClientMeta):
    ...
    @staticmethod
    def policy_tag_path(project: str,location: str,taxonomy: str,policy_tag: str,) -> str:
        """Return a fully-qualified policy_tag string."""
        return "projects/{project}/locations/{location}/taxonomies/{taxonomy}/policyTags/{policy_tag}".format(project=project, location=location, taxonomy=taxonomy, policy_tag=policy_tag, )

    @staticmethod
    def parse_policy_tag_path(path: str) -> Dict[str,str]:
        """Parse a policy_tag path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/taxonomies/(?P<taxonomy>.+?)/policyTags/(?P<policy_tag>.+?)$", path)
        return m.groupdict() if m else {}

    @staticmethod
    def taxonomy_path(project: str,location: str,taxonomy: str,) -> str:
        """Return a fully-qualified taxonomy string."""
        return "projects/{project}/locations/{location}/taxonomies/{taxonomy}".format(project=project, location=location, taxonomy=taxonomy, )

    @staticmethod
    def parse_taxonomy_path(path: str) -> Dict[str,str]:
        """Parse a taxonomy path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/taxonomies/(?P<taxonomy>.+?)$", path)
        return m.groupdict() if m else {}
