# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/cloud/datacatalog/v1beta1/gcs_fileset_spec.proto

require 'google/protobuf'

require 'google/api/field_behavior_pb'
require 'google/cloud/datacatalog/v1beta1/timestamps_pb'
Google::Protobuf::DescriptorPool.generated_pool.build do
  add_file("google/cloud/datacatalog/v1beta1/gcs_fileset_spec.proto", :syntax => :proto3) do
    add_message "google.cloud.datacatalog.v1beta1.GcsFilesetSpec" do
      repeated :file_patterns, :string, 1
      repeated :sample_gcs_file_specs, :message, 2, "google.cloud.datacatalog.v1beta1.GcsFileSpec"
    end
    add_message "google.cloud.datacatalog.v1beta1.GcsFileSpec" do
      optional :file_path, :string, 1
      optional :gcs_timestamps, :message, 2, "google.cloud.datacatalog.v1beta1.SystemTimestamps"
      optional :size_bytes, :int64, 4
    end
  end
end

module Google
  module Cloud
    module DataCatalog
      module V1beta1
        GcsFilesetSpec = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.cloud.datacatalog.v1beta1.GcsFilesetSpec").msgclass
        GcsFileSpec = ::Google::Protobuf::DescriptorPool.generated_pool.lookup("google.cloud.datacatalog.v1beta1.GcsFileSpec").msgclass
      end
    end
  end
end
