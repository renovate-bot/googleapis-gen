...
namespace Google.Cloud.Automl.V1Beta1.Snippets
{
    ...
    public sealed partial class GeneratedAutoMlClientStandaloneSnippets
    {
        ...
        public async Task ExportEvaluatedExamplesAsync()
        {
            // Snippet: ExportEvaluatedExamplesAsync(string, ExportEvaluatedExamplesOutputConfig, CallSettings)
            // Additional: ExportEvaluatedExamplesAsync(string, ExportEvaluatedExamplesOutputConfig, CancellationToken)
            // Create client
            ...
            // Make the request
            Operation<Empty, OperationMetadata> response = await autoMlClient.ExportEvaluatedExamplesAsync(name, outputConfig);
