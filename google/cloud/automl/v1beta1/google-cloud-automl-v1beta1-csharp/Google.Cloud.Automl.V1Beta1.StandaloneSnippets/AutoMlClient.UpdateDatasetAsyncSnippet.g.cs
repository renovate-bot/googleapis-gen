...
namespace Google.Cloud.Automl.V1Beta1.Snippets
{
    ...
    public sealed partial class GeneratedAutoMlClientStandaloneSnippets
    {
        ...
        {
            // Snippet: UpdateDatasetAsync(Dataset, CallSettings)
            // Additional: UpdateDatasetAsync(Dataset, CancellationToken)
            // Create client
            AutoMlClient autoMlClient = await AutoMlClient.CreateAsync();
            // Initialize request argument(s)
            Dataset dataset = new Dataset();
            // Make the request
            Dataset response = await autoMlClient.UpdateDatasetAsync(dataset);
            // End snippet
        }
    }
}
