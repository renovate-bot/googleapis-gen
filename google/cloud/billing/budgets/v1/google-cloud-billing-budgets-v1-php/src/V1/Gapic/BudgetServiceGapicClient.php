 ...
 * https://github.com/google/googleapis/blob/master/google/cloud/billing/budgets/v1/budget_service.proto
...
class BudgetServiceGapicClient
{
    ...
    {
        if (null == self::$budgetNameTemplate) {
    ...
    public function updateBudget($budget, array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor([
          'budget.name' => $request->getBudget()->getName(),
        ]);
        $optionalArgs['headers'] = isset($optionalArgs['headers'])
            ? array_merge($requestParams->getHeader(), $optionalArgs['headers'])
            : $requestParams->getHeader();
