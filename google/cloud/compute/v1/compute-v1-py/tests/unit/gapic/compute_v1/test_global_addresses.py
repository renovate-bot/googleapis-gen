...
from google.cloud.compute_v1.services.global_addresses import transports
...
def test_global_addresses_client_from_service_account_info(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.GlobalAddressesRestTransport, "rest"),
])
...
def test_global_addresses_client_from_service_account_file(client_class):
    creds = ga_credentials.AnonymousCredentials()
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (GlobalAddressesClient, transports.GlobalAddressesRestTransport, "rest"),
])
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,use_client_cert_env", [
    (GlobalAddressesClient, transports.GlobalAddressesRestTransport, "rest", "true"),
    (GlobalAddressesClient, transports.GlobalAddressesRestTransport, "rest", "false"),
])
@mock.patch.object(GlobalAddressesClient, "DEFAULT_ENDPOINT", modify_default_endpoint(GlobalAddressesClient))
...
def test_global_addresses_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_delete_rest(transport: str = 'rest', request_type=compute.DeleteGlobalAddressRequest):
...
def test_delete_rest_flattened(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert path_template.validate("https://%s/compute/v1/projects/{project}/global/addresses/{address}" % client.transport._host, args[1])
...
def test_insert_rest(transport: str = 'rest', request_type=compute.InsertGlobalAddressRequest):
    client = GlobalAddressesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    request_init["address_resource"] = compute.Address(address='address_value')
    request = request_type(request_init)
...
def test_insert_rest_flattened(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("https://%s/compute/v1/projects/{project}/global/addresses" % client.transport._host, args[1])
...
def test_list_rest(transport: str = 'rest', request_type=compute.ListGlobalAddressesRequest):
    client = GlobalAddressesClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        json_return_value = compute.AddressList.to_json(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
...
def test_list_rest_pager():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        #with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            assert all(isinstance(i, compute.Address)
                    for i in results)
...
def test_global_addresses_host_no_port():
    client = GlobalAddressesClient(
        credentials=ga_credentials.AnonymousCredentials(),
...
def test_global_addresses_host_with_port():
    client = GlobalAddressesClient(
        credentials=ga_credentials.AnonymousCredentials(),
