...
def test__get_default_mtls_endpoint():
    ...
    assert FirewallsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert FirewallsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert FirewallsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test_firewalls_client_service_account_always_use_jwt(transport_class, transport_name):
...
@mock.patch.object(FirewallsClient, "DEFAULT_ENDPOINT", modify_default_endpoint(FirewallsClient))
...
def test_firewalls_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_firewalls_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_firewalls_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_insert_rest(transport: str = 'rest', request_type=compute.InsertFirewallRequest):
    ...
    request_init["firewall_resource"] = compute.Firewall(allowed=[compute.Allowed(I_p_protocol='I_p_protocol_value')])
    request = request_type(request_init)
...
def test_list_rest_pager():
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        #with mock.patch.object(path_template, 'transcode') as transcode:
            ...
            assert all(isinstance(i, compute.Firewall)
                    for i in results)
...
def test_firewalls_base_transport():
    ...
    with mock.patch('google.cloud.compute_v1.services.firewalls.transports.FirewallsTransport.__init__') as Transport:
...
def test_firewalls_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_firewalls_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.firewalls.transports.FirewallsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.FirewallsTransport()
        adc.assert_called_once()
...
def test_client_withDEFAULT_CLIENT_INFO():
    ...
    with mock.patch.object(transports.FirewallsTransport, '_prep_wrapped_messages') as prep:
        client = FirewallsClient(
            credentials=ga_credentials.AnonymousCredentials(),
