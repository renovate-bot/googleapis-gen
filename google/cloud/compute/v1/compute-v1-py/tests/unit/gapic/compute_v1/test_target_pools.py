...
def test_target_pools_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_aggregated_list_rest_flattened(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("https://%s/compute/v1/projects/{project}/aggregated/targetPools" % client.transport._host, args[1])
...
def test_get_health_rest_flattened(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("https://%s/compute/v1/projects/{project}/regions/{region}/targetPools/{target_pool}/getHealth" % client.transport._host, args[1])
...
def test_target_pools_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.compute_v1.services.target_pools.transports.TargetPoolsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_target_pools_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.compute_v1.services.target_pools.transports.TargetPoolsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.TargetPoolsTransport()
        adc.assert_called_once()
