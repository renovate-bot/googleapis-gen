<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: google/cloud/documentai/v1beta1/document_understanding.proto

namespace Google\Cloud\DocumentAI\V1beta1;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Request to process one document.
 *
 * Generated from protobuf message <code>google.cloud.documentai.v1beta1.ProcessDocumentRequest</code>
 */
class ProcessDocumentRequest extends \Google\Protobuf\Internal\Message
{
    /**
     * Required. Information about the input file.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.InputConfig input_config = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     */
    protected $input_config = null;
    /**
     * Required. The desired output location.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OutputConfig output_config = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     */
    protected $output_config = null;
    /**
     * Specifies a known document type for deeper structure detection. Valid
     * values are currently "general" and "invoice". If not provided, "general"\
     * is used as default. If any other value is given, the request is rejected.
     *
     * Generated from protobuf field <code>string document_type = 3;</code>
     */
    protected $document_type = '';
    /**
     * Controls table extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.TableExtractionParams table_extraction_params = 4;</code>
     */
    protected $table_extraction_params = null;
    /**
     * Controls form extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.FormExtractionParams form_extraction_params = 5;</code>
     */
    protected $form_extraction_params = null;
    /**
     * Controls entity extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.EntityExtractionParams entity_extraction_params = 6;</code>
     */
    protected $entity_extraction_params = null;
    /**
     * Controls OCR behavior. If not specified, the system will decide reasonable
     * defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OcrParams ocr_params = 7;</code>
     */
    protected $ocr_params = null;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type \Google\Cloud\DocumentAI\V1beta1\InputConfig $input_config
     *           Required. Information about the input file.
     *     @type \Google\Cloud\DocumentAI\V1beta1\OutputConfig $output_config
     *           Required. The desired output location.
     *     @type string $document_type
     *           Specifies a known document type for deeper structure detection. Valid
     *           values are currently "general" and "invoice". If not provided, "general"\
     *           is used as default. If any other value is given, the request is rejected.
     *     @type \Google\Cloud\DocumentAI\V1beta1\TableExtractionParams $table_extraction_params
     *           Controls table extraction behavior. If not specified, the system will
     *           decide reasonable defaults.
     *     @type \Google\Cloud\DocumentAI\V1beta1\FormExtractionParams $form_extraction_params
     *           Controls form extraction behavior. If not specified, the system will
     *           decide reasonable defaults.
     *     @type \Google\Cloud\DocumentAI\V1beta1\EntityExtractionParams $entity_extraction_params
     *           Controls entity extraction behavior. If not specified, the system will
     *           decide reasonable defaults.
     *     @type \Google\Cloud\DocumentAI\V1beta1\OcrParams $ocr_params
     *           Controls OCR behavior. If not specified, the system will decide reasonable
     *           defaults.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Google\Cloud\Documentai\V1Beta1\DocumentUnderstanding::initOnce();
        parent::__construct($data);
    }

    /**
     * Required. Information about the input file.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.InputConfig input_config = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\InputConfig
     */
    public function getInputConfig()
    {
        return isset($this->input_config) ? $this->input_config : null;
    }

    public function hasInputConfig()
    {
        return isset($this->input_config);
    }

    public function clearInputConfig()
    {
        unset($this->input_config);
    }

    /**
     * Required. Information about the input file.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.InputConfig input_config = 1 [(.google.api.field_behavior) = REQUIRED];</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\InputConfig $var
     * @return $this
     */
    public function setInputConfig($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\InputConfig::class);
        $this->input_config = $var;

        return $this;
    }

    /**
     * Required. The desired output location.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OutputConfig output_config = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\OutputConfig
     */
    public function getOutputConfig()
    {
        return isset($this->output_config) ? $this->output_config : null;
    }

    public function hasOutputConfig()
    {
        return isset($this->output_config);
    }

    public function clearOutputConfig()
    {
        unset($this->output_config);
    }

    /**
     * Required. The desired output location.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OutputConfig output_config = 2 [(.google.api.field_behavior) = REQUIRED];</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\OutputConfig $var
     * @return $this
     */
    public function setOutputConfig($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\OutputConfig::class);
        $this->output_config = $var;

        return $this;
    }

    /**
     * Specifies a known document type for deeper structure detection. Valid
     * values are currently "general" and "invoice". If not provided, "general"\
     * is used as default. If any other value is given, the request is rejected.
     *
     * Generated from protobuf field <code>string document_type = 3;</code>
     * @return string
     */
    public function getDocumentType()
    {
        return $this->document_type;
    }

    /**
     * Specifies a known document type for deeper structure detection. Valid
     * values are currently "general" and "invoice". If not provided, "general"\
     * is used as default. If any other value is given, the request is rejected.
     *
     * Generated from protobuf field <code>string document_type = 3;</code>
     * @param string $var
     * @return $this
     */
    public function setDocumentType($var)
    {
        GPBUtil::checkString($var, True);
        $this->document_type = $var;

        return $this;
    }

    /**
     * Controls table extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.TableExtractionParams table_extraction_params = 4;</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\TableExtractionParams
     */
    public function getTableExtractionParams()
    {
        return isset($this->table_extraction_params) ? $this->table_extraction_params : null;
    }

    public function hasTableExtractionParams()
    {
        return isset($this->table_extraction_params);
    }

    public function clearTableExtractionParams()
    {
        unset($this->table_extraction_params);
    }

    /**
     * Controls table extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.TableExtractionParams table_extraction_params = 4;</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\TableExtractionParams $var
     * @return $this
     */
    public function setTableExtractionParams($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\TableExtractionParams::class);
        $this->table_extraction_params = $var;

        return $this;
    }

    /**
     * Controls form extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.FormExtractionParams form_extraction_params = 5;</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\FormExtractionParams
     */
    public function getFormExtractionParams()
    {
        return isset($this->form_extraction_params) ? $this->form_extraction_params : null;
    }

    public function hasFormExtractionParams()
    {
        return isset($this->form_extraction_params);
    }

    public function clearFormExtractionParams()
    {
        unset($this->form_extraction_params);
    }

    /**
     * Controls form extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.FormExtractionParams form_extraction_params = 5;</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\FormExtractionParams $var
     * @return $this
     */
    public function setFormExtractionParams($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\FormExtractionParams::class);
        $this->form_extraction_params = $var;

        return $this;
    }

    /**
     * Controls entity extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.EntityExtractionParams entity_extraction_params = 6;</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\EntityExtractionParams
     */
    public function getEntityExtractionParams()
    {
        return isset($this->entity_extraction_params) ? $this->entity_extraction_params : null;
    }

    public function hasEntityExtractionParams()
    {
        return isset($this->entity_extraction_params);
    }

    public function clearEntityExtractionParams()
    {
        unset($this->entity_extraction_params);
    }

    /**
     * Controls entity extraction behavior. If not specified, the system will
     * decide reasonable defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.EntityExtractionParams entity_extraction_params = 6;</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\EntityExtractionParams $var
     * @return $this
     */
    public function setEntityExtractionParams($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\EntityExtractionParams::class);
        $this->entity_extraction_params = $var;

        return $this;
    }

    /**
     * Controls OCR behavior. If not specified, the system will decide reasonable
     * defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OcrParams ocr_params = 7;</code>
     * @return \Google\Cloud\DocumentAI\V1beta1\OcrParams
     */
    public function getOcrParams()
    {
        return isset($this->ocr_params) ? $this->ocr_params : null;
    }

    public function hasOcrParams()
    {
        return isset($this->ocr_params);
    }

    public function clearOcrParams()
    {
        unset($this->ocr_params);
    }

    /**
     * Controls OCR behavior. If not specified, the system will decide reasonable
     * defaults.
     *
     * Generated from protobuf field <code>.google.cloud.documentai.v1beta1.OcrParams ocr_params = 7;</code>
     * @param \Google\Cloud\DocumentAI\V1beta1\OcrParams $var
     * @return $this
     */
    public function setOcrParams($var)
    {
        GPBUtil::checkMessage($var, \Google\Cloud\DocumentAI\V1beta1\OcrParams::class);
        $this->ocr_params = $var;

        return $this;
    }

}

