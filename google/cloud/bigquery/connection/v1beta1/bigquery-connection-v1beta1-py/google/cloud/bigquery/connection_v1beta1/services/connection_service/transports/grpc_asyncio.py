...
class ConnectionServiceGrpcAsyncIOTransport(ConnectionServiceTransport):
    ...
    def update_connection_credential(self) -> Callable[
        ...
        Returns:
            Callable[[~.UpdateConnectionCredentialRequest],
                    Awaitable[~.Empty]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'update_connection_credential' not in self._stubs:
            self._stubs['update_connection_credential'] = self.grpc_channel.unary_unary(
                '/google.cloud.bigquery.connection.v1beta1.ConnectionService/UpdateConnectionCredential',
                request_serializer=connection.UpdateConnectionCredentialRequest.serialize,
                response_deserializer=empty_pb2.Empty.FromString,
            )
        return self._stubs['update_connection_credential']
