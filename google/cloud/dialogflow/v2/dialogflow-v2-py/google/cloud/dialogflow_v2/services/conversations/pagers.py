...
class ListCallMatchersPager:
    """A pager for iterating through ``list_call_matchers`` requests.

    This class thinly wraps an initial
    :class:`google.cloud.dialogflow_v2.types.ListCallMatchersResponse` object, and
    provides an ``__iter__`` method to iterate through its
    ``call_matchers`` field.

    If there are more pages, the ``__iter__`` method will make additional
    ``ListCallMatchers`` requests and continue to iterate
    through the ``call_matchers`` field on the
    corresponding responses.

    All the usual :class:`google.cloud.dialogflow_v2.types.ListCallMatchersResponse`
    attributes are available on the pager. If multiple requests are made, only
    the most recent response is retained, and thus used for attribute lookup.
    """
    def __init__(self,
            method: Callable[..., conversation.ListCallMatchersResponse],
            request: conversation.ListCallMatchersRequest,
            response: conversation.ListCallMatchersResponse,
            *,
            metadata: Sequence[Tuple[str, str]] = ()):
        """Instantiate the pager.
        ...
        Args:
            ...
            request (google.cloud.dialogflow_v2.types.ListCallMatchersRequest):
                The initial request object.
            response (google.cloud.dialogflow_v2.types.ListCallMatchersResponse):
                The initial response object.
            metadata (Sequence[Tuple[str, str]]): Strings which should be
                sent along with the request as metadata.
        """
        self._method = method
        self._request = conversation.ListCallMatchersRequest(request)
        self._response = response
        self._metadata = metadata
    ...
    @property
    def pages(self) -> Iterable[conversation.ListCallMatchersResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = self._method(self._request, metadata=self._metadata)
    ...
    def __iter__(self) -> Iterable[conversation.CallMatcher]:
        for page in self.pages:
            yield from page.call_matchers

    def __repr__(self) -> str:
        return '{0}<{1!r}>'.format(self.__class__.__name__, self._response)


class ListCallMatchersAsyncPager:
    """A pager for iterating through ``list_call_matchers`` requests.
    ...
    """
    def __init__(self,
            method: Callable[..., Awaitable[conversation.ListCallMatchersResponse]],
            request: conversation.ListCallMatchersRequest,
            response: conversation.ListCallMatchersResponse,
            *,
            metadata: Sequence[Tuple[str, str]] = ()):
    ...
    @property
    async def pages(self) -> AsyncIterable[conversation.ListCallMatchersResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, metadata=self._metadata)
    ...
    def __aiter__(self) -> AsyncIterable[conversation.CallMatcher]:
        async def async_generator():
            async for page in self.pages:
                for response in page.call_matchers:
                    yield response
