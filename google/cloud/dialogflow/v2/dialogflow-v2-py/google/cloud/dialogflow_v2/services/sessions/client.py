...
class SessionsClient(metaclass=SessionsClientMeta):
    ...
    def session_path(project: str,session: str,) -> str:
        ...
        return "projects/{project}/agent/sessions/{session}".format(project=project, session=session, )

    @staticmethod
    def parse_session_path(path: str) -> Dict[str,str]:
        """Parse a session path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/agent/sessions/(?P<session>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def detect_intent(self,
        ...
        Args:
            ...
            session (str):
                ...
                specified, we are using "-". It's up to the API caller
                to choose an appropriate ``Session ID`` and ``User Id``.
                They can be a random number or some type of user and
                session identifiers (preferably hashed). The length of
                the ``Session ID`` and ``User ID`` must not exceed 36
                characters.
