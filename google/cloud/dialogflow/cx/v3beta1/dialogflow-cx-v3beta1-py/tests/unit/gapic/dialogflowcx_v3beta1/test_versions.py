...
from google.cloud.dialogflowcx_v3beta1.services.versions.transports.base import _API_CORE_VERSION
from google.cloud.dialogflowcx_v3beta1.services.versions.transports.base import _GOOGLE_AUTH_VERSION
...
def test__get_default_mtls_endpoint():
    ...
    assert VersionsClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert VersionsClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert VersionsClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
...
def test_versions_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_versions_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_versions_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_versions_client_client_options_from_dict():
    with mock.patch('google.cloud.dialogflowcx_v3beta1.services.versions.transports.VersionsGrpcTransport.__init__') as grpc_transport:
...
def test_list_versions(transport: str = 'grpc', request_type=version.ListVersionsRequest):
    client = VersionsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_list_versions_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, version.Version)
                   for i in results)
...
def test_create_version(transport: str = 'grpc', request_type=gcdc_version.CreateVersionRequest):
    client = VersionsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_update_version(transport: str = 'grpc', request_type=gcdc_version.UpdateVersionRequest):
    client = VersionsClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
async def test_update_version_async(transport: str = 'grpc_asyncio', request_type=gcdc_version.UpdateVersionRequest):
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(gcdc_version.Version(
            name='name_value',
            display_name='display_name_value',
            description='description_value',
            state=gcdc_version.Version.State.RUNNING,
...
def test_versions_base_transport():
    ...
    with mock.patch('google.cloud.dialogflowcx_v3beta1.services.versions.transports.VersionsTransport.__init__') as Transport:
...
def test_versions_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.dialogflowcx_v3beta1.services.versions.transports.VersionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_versions_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.dialogflowcx_v3beta1.services.versions.transports.VersionsTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.VersionsTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.VersionsGrpcTransport, transports.VersionsGrpcAsyncIOTransport])
def test_versions_grpc_transport_client_cert_source_for_mtls(
    transport_class
):
...
@pytest.mark.parametrize("transport_class", [transports.VersionsGrpcTransport, transports.VersionsGrpcAsyncIOTransport])
def test_versions_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.VersionsGrpcTransport, transports.VersionsGrpcAsyncIOTransport])
def test_versions_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
...
def test_client_withDEFAULT_CLIENT_INFO():
    ...
    with mock.patch.object(transports.VersionsTransport, '_prep_wrapped_messages') as prep:
        client = VersionsClient(
            credentials=credentials.AnonymousCredentials(),
