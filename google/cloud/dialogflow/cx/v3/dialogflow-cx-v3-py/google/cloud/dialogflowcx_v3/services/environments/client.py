...
class EnvironmentsClient(metaclass=EnvironmentsClientMeta):
    ...
    @staticmethod
    def version_path(project: str,location: str,agent: str,flow: str,version: str,) -> str:
        """Return a fully-qualified version string."""
        return "projects/{project}/locations/{location}/agents/{agent}/flows/{flow}/versions/{version}".format(project=project, location=location, agent=agent, flow=flow, version=version, )

    @staticmethod
    def parse_version_path(path: str) -> Dict[str,str]:
        """Parse a version path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/agents/(?P<agent>.+?)/flows/(?P<flow>.+?)/versions/(?P<version>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def lookup_environment_history(self,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.lookup_environment_history]
