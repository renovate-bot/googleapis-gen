 ...
 * https://github.com/google/googleapis/blob/master/google/cloud/dialogflow/cx/v3/version.proto
...
use Google\Cloud\Dialogflow\Cx\V3\ListVersionsResponse;
...
class VersionsGapicClient
{
     ...
     */
    const SERVICE_NAME = 'google.cloud.dialogflow.cx.v3.Versions';
    ...
    {
        if (null == self::$versionNameTemplate) {
    ...
    public function updateVersion($version, $updateMask, array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor([
          'version.name' => $request->getVersion()->getName(),
        ]);
        $optionalArgs['headers'] = isset($optionalArgs['headers'])
            ? array_merge($requestParams->getHeader(), $optionalArgs['headers'])
            : $requestParams->getHeader();
