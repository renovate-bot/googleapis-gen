 ...
 * https://github.com/google/googleapis/blob/master/google/cloud/dialogflow/cx/v3/session.proto
...
use Google\Cloud\Dialogflow\Cx\V3\Match;
...
class SessionsGapicClient
{
     ...
     *     $session = '';
     *     $queryInput = new QueryInput();
     *     $response = $sessionsClient->matchIntent($session, $queryInput);
     * } finally {
     *     $sessionsClient->close();
     * }
     * ```
    ...
    public function fulfillIntent(array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor([
          'match_intent_request.session' => $request->getMatchIntentRequest()->getSession(),
        ]);
        $optionalArgs['headers'] = isset($optionalArgs['headers'])
            ? array_merge($requestParams->getHeader(), $optionalArgs['headers'])
            : $requestParams->getHeader();
