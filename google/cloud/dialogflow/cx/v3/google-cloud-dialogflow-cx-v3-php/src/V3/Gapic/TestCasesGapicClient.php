...
use Google\Cloud\Dialogflow\Cx\V3\ExportTestCasesRequest\DataFormat;
...
class TestCasesGapicClient
{
    ...
    {
        if (null == self::$testCaseNameTemplate) {
    ...
    public function updateTestCase($testCase, $updateMask, array $optionalArgs = [])
    {
        ...
        $requestParams = new RequestParamsHeaderDescriptor([
          'test_case.name' => $request->getTestCase()->getName(),
        ]);
        $optionalArgs['headers'] = isset($optionalArgs['headers'])
            ? array_merge($requestParams->getHeader(), $optionalArgs['headers'])
            : $requestParams->getHeader();
    ...
    public function calculateCoverage($agent, $type, array $optionalArgs = [])
    {
        ...
        return $this->startCall(
            'CalculateCoverage',
            CalculateCoverageResponse::class,
            $optionalArgs,
            $request
        )->wait();
    }
