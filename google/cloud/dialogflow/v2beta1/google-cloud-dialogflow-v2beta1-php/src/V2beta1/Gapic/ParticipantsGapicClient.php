 ...
 * https://github.com/google/googleapis/blob/master/google/cloud/dialogflow/v2beta1/participant.proto
...
use Google\Cloud\Dialogflow\V2beta1\SuggestSmartRepliesRequest;
use Google\Cloud\Dialogflow\V2beta1\SuggestSmartRepliesResponse;
use Google\Cloud\Dialogflow\V2beta1\TextInput;
use Google\Cloud\Dialogflow\V2beta1\UpdateParticipantRequest;
...
class ParticipantsGapicClient
{
     ...
     */
    const SERVICE_NAME = 'google.cloud.dialogflow.v2beta1.Participants';
     ...
     * try {
     *     $parent = '';
     *     $response = $participantsClient->suggestSmartReplies($parent);
     * } finally {
     *     $participantsClient->close();
     * }
     * ```
    ...
    public function suggestSmartReplies($parent, array $optionalArgs = [])
    {
        ...
        return $this->startCall(
            'SuggestSmartReplies',
            SuggestSmartRepliesResponse::class,
            $optionalArgs,
            $request
        )->wait();
    }
     ...
     * try {
     *     // Iterate over pages of elements
     *     $pagedResponse = $participantsClient->listSuggestions();
     *     foreach ($pagedResponse->iteratePages() as $page) {
     *         foreach ($page as $element) {
     *             // doSomethingWith($element);
     *         }
     ...
     * @experimental
     */
    public function listSuggestions(array $optionalArgs = [])
    {
        $request = new ListSuggestionsRequest();
     ...
     * $participantsClient = new ParticipantsClient();
     * try {
     *     $response = $participantsClient->compileSuggestion();
     * } finally {
     *     $participantsClient->close();
     * }
     * ```
     ...
     * @experimental
     */
    public function compileSuggestion(array $optionalArgs = [])
    {
        $request = new CompileSuggestionRequest();
        if (isset($optionalArgs['parent'])) {
            $request->setParent($optionalArgs['parent']);
        }
        if (isset($optionalArgs['latestMessage'])) {
        ...
        return $this->startCall(
            'CompileSuggestion',
            CompileSuggestionResponse::class,
            $optionalArgs,
            $request
        )->wait();
    }
