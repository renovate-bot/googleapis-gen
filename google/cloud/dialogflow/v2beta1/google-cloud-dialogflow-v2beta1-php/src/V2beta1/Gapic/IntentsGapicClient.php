 ...
 *     $pagedResponse = $intentsClient->listIntents($parent);
...
class IntentsGapicClient
{
    ...
    {
        if (null == self::$projectIntentNameTemplate) {
    ...
    {
        if (null == self::$projectLocationIntentNameTemplate) {
    ...
    /**
     * Formats a string containing the fully-qualified path to represent
     * a project_location_intent resource.
     *
     * @param string $project
     * @param string $location
     * @param string $intent
     ...
     * try {
     *     $name = '';
     *     $response = $intentsClient->getIntent($name);
     * } finally {
     *     $intentsClient->close();
     * }
     * ```
     ...
     * try {
     *     $parent = '';
     *     $operationResponse = $intentsClient->batchUpdateIntents($parent);
     ...
     *     $parent = '';
     *     $intents = [];
     *     $operationResponse = $intentsClient->batchDeleteIntents($parent, $intents);
     *     $operationResponse->pollUntilComplete();
     *     if ($operationResponse->operationSucceeded()) {
