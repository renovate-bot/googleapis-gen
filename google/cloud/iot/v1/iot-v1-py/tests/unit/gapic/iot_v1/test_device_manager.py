...
def test__get_default_mtls_endpoint():
    ...
    assert DeviceManagerClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert DeviceManagerClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert DeviceManagerClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert DeviceManagerClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert DeviceManagerClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi


def test_device_manager_client_from_service_account_info():
    creds = credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        factory.return_value = creds
        info = {"valid": True}
        client = DeviceManagerClient.from_service_account_info(info)
...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    ...
    (DeviceManagerAsyncClient, transports.DeviceManagerGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_device_manager_client_client_options(client_class, transport_class, transport_name):
...
def test_device_manager_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
...
def test_device_manager_client_client_options_scopes(client_class, transport_class, transport_name):
...
def test_device_manager_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_device_manager_client_client_options_from_dict():
    with mock.patch('google.cloud.iot_v1.services.device_manager.transports.DeviceManagerGrpcTransport.__init__') as grpc_transport:
...
def test_list_device_registries_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, resources.DeviceRegistry)
                   for i in results)
...
def test_list_devices_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in pager]
        assert len(results) == 6
        assert all(isinstance(i, resources.Device)
                   for i in results)
...
@pytest.mark.parametrize("transport_class", [transports.DeviceManagerGrpcTransport, transports.DeviceManagerGrpcAsyncIOTransport])
...
@pytest.mark.parametrize("transport_class", [transports.DeviceManagerGrpcTransport, transports.DeviceManagerGrpcAsyncIOTransport])
def test_device_manager_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.DeviceManagerGrpcTransport, transports.DeviceManagerGrpcAsyncIOTransport])
def test_device_manager_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
