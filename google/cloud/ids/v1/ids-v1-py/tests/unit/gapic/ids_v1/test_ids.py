...
from google.cloud.ids_v1.services.ids.transports.base import _GOOGLE_AUTH_VERSION
...
def test_ids_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_create_endpoint(transport: str = 'grpc', request_type=ids.CreateEndpointRequest):
    client = IDSClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_create_endpoint_flattened():
    ...
    with mock.patch.object(
        ...
        _, args, _ = call.mock_calls[0]
        assert args[0].parent == 'parent_value'
        assert args[0].endpoint == ids.Endpoint(name='name_value')
...
def test_delete_endpoint(transport: str = 'grpc', request_type=ids.DeleteEndpointRequest):
    client = IDSClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_ids_base_transport_with_credentials_file_old_google_auth():
    # Instantiate the base transport with a credentials file
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.ids_v1.services.ids.transports.IDSTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@requires_google_auth_lt_1_25_0
def test_ids_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        IDSClient()
...
@requires_google_auth_lt_1_25_0
def test_ids_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
