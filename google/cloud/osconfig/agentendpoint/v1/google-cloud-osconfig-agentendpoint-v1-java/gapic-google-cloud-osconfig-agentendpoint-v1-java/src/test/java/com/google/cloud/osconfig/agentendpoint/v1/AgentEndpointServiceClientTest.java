...
public class AgentEndpointServiceClientTest {
  ...
  @Test
  public void reportInventoryTest() throws Exception {
    ReportInventoryResponse expectedResponse =
        ReportInventoryResponse.newBuilder().setReportFullInventory(true).build();
    mockAgentEndpointService.addResponse(expectedResponse);
    ...
    ReportInventoryResponse actualResponse =
        client.reportInventory(instanceIdToken, inventoryChecksum, inventory);
    Assert.assertEquals(expectedResponse, actualResponse);

    List<AbstractMessage> actualRequests = mockAgentEndpointService.getRequests();
    Assert.assertEquals(1, actualRequests.size());
    ReportInventoryRequest actualRequest = ((ReportInventoryRequest) actualRequests.get(0));

    Assert.assertEquals(instanceIdToken, actualRequest.getInstanceIdToken());
    Assert.assertEquals(inventoryChecksum, actualRequest.getInventoryChecksum());
  ...
  @Test
  public void reportInventoryExceptionTest() throws Exception {
    StatusRuntimeException exception = new StatusRuntimeException(io.grpc.Status.INVALID_ARGUMENT);
    mockAgentEndpointService.addException(exception);
