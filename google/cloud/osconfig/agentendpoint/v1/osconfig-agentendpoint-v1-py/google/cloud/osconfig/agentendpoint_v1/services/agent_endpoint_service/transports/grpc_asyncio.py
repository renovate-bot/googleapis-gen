...
class AgentEndpointServiceGrpcAsyncIOTransport(AgentEndpointServiceTransport):
    """gRPC AsyncIO backend transport for AgentEndpointService.
    ...
    def receive_task_notification(self) -> Callable[
        ...
        Returns:
            Callable[[~.ReceiveTaskNotificationRequest],
                    Awaitable[~.ReceiveTaskNotificationResponse]]:
    ...
    def start_next_task(self) -> Callable[
        ...
        Returns:
            Callable[[~.StartNextTaskRequest],
                    Awaitable[~.StartNextTaskResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def report_task_progress(self) -> Callable[
        ...
        Returns:
            Callable[[~.ReportTaskProgressRequest],
                    Awaitable[~.ReportTaskProgressResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def report_task_complete(self) -> Callable[
        ...
        Returns:
            Callable[[~.ReportTaskCompleteRequest],
                    Awaitable[~.ReportTaskCompleteResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def register_agent(self) -> Callable[
        ...
        Returns:
            Callable[[~.RegisterAgentRequest],
                    Awaitable[~.RegisterAgentResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def report_inventory(self) -> Callable[
        ...
        Returns:
            Callable[[~.ReportInventoryRequest],
                    Awaitable[~.ReportInventoryResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if 'report_inventory' not in self._stubs:
            self._stubs['report_inventory'] = self.grpc_channel.unary_unary(
                '/google.cloud.osconfig.agentendpoint.v1.AgentEndpointService/ReportInventory',
                request_serializer=agentendpoint.ReportInventoryRequest.serialize,
                response_deserializer=agentendpoint.ReportInventoryResponse.deserialize,
            )
