...
@pytest.mark.parametrize("client_class,transport_class,transport_name", [
    (ResourceSettingsServiceClient, transports.ResourceSettingsServiceGrpcTransport, "grpc"),
    (ResourceSettingsServiceAsyncClient, transports.ResourceSettingsServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
...
def test_resource_settings_service_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_list_settings_from_dict():
    test_list_settings(request_type=dict)
...
def test_list_settings_pager():
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, resource_settings.Setting)
                   for i in results)
...
def test_resource_settings_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.resourcesettings_v1.services.resource_settings_service.transports.ResourceSettingsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_resource_settings_service_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.cloud.resourcesettings_v1.services.resource_settings_service.transports.ResourceSettingsServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ResourceSettingsServiceTransport()
        adc.assert_called_once()
...
@pytest.mark.parametrize("transport_class", [transports.ResourceSettingsServiceGrpcTransport, transports.ResourceSettingsServiceGrpcAsyncIOTransport])
def test_resource_settings_service_transport_channel_mtls_with_client_cert_source(
    transport_class
):
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ResourceSettingsServiceGrpcTransport, transports.ResourceSettingsServiceGrpcAsyncIOTransport])
def test_resource_settings_service_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
