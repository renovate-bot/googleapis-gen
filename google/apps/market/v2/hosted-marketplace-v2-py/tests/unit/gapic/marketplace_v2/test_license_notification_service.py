...
from ccc.hosted.marketplace_v2.services.license_notification_service.transports.base import _GOOGLE_AUTH_VERSION
...
def test_license_notification_service_client_service_account_always_use_jwt(client_class):
...
@pytest.mark.parametrize("transport_class,transport_name", [
    (transports.LicenseNotificationServiceGrpcTransport, "grpc"),
    (transports.LicenseNotificationServiceGrpcAsyncIOTransport, "grpc_asyncio"),
])
def test_license_notification_service_client_service_account_always_use_jwt_true(transport_class, transport_name):
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
...
def test_license_notification_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('ccc.hosted.marketplace_v2.services.license_notification_service.transports.LicenseNotificationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_license_notification_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('ccc.hosted.marketplace_v2.services.license_notification_service.transports.LicenseNotificationServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.LicenseNotificationServiceTransport()
        adc.assert_called_once()
...
@requires_google_auth_lt_1_25_0
def test_license_notification_service_auth_adc_old_google_auth():
    # If no credentials are provided, we should use ADC credentials.
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        LicenseNotificationServiceClient()
...
@requires_google_auth_gte_1_25_0
def test_license_notification_service_transport_auth_adc(transport_class):
...
@requires_google_auth_lt_1_25_0
def test_license_notification_service_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
...
@pytest.mark.parametrize(
    ...
    [
        (transports.LicenseNotificationServiceGrpcTransport, grpc_helpers),
        (transports.LicenseNotificationServiceGrpcAsyncIOTransport, grpc_helpers_async)
    ],
)
def test_license_notification_service_transport_create_channel(transport_class, grpc_helpers):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
