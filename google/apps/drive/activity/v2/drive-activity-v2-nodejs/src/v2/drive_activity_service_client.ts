 ...
 * `src/v2/drive_activity_service_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
import * as gapicConfig from './drive_activity_service_client_config.json';
 ...
 * @memberof v2
 */
export class DriveActivityServiceClient {
  ...
  driveActivityServiceStub?: Promise<{[name: string]: Function}>;

  /**
   * Construct an instance of DriveActivityServiceClient.
   *
  ...
  constructor(opts?: ClientOptions) {
    ...
    const staticMembers = this.constructor as typeof DriveActivityServiceClient;
    ...
    this.descriptors.page = {
      queryDriveActivity:
          new this._gaxModule.PageDescriptor('pageToken', 'nextPageToken', 'activities')
    };

    // Put together the default options sent with requests.
    this._defaults = this._gaxGrpc.constructSettings(
        'google.apps.drive.activity.v2.DriveActivityService', gapicConfig as gax.ClientConfig,
  ...
  initialize() {
    ...
    if (this.driveActivityServiceStub) {
      return this.driveActivityServiceStub;
    }

    // Put together the "service stub" for
    // google.apps.drive.activity.v2.DriveActivityService.
    this.driveActivityServiceStub = this._gaxGrpc.createStub(
        this._opts.fallback ?
          (this._protos as protobuf.Root).lookupService('google.apps.drive.activity.v2.DriveActivityService') :
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this._protos as any).google.apps.drive.activity.v2.DriveActivityService,
        this._opts, this._providedCustomServicePath) as Promise<{[method: string]: Function}>;

    // Iterate over each of the methods that the service provides
    // and create an API call method for each.
    const driveActivityServiceStubMethods =
        ['queryDriveActivity'];
    for (const methodName of driveActivityServiceStubMethods) {
      const callPromise = this.driveActivityServiceStub.then(
        stub => (...args: Array<{}>) => {
          if (this._terminated) {
  ...
  static get servicePath() {
    return 'driveactivity.googleapis.com';
  }
 ...
 * @param {string} request.itemName
 *   Return activities for this Drive item. The format is
 *   `items/ITEM_ID`.
 * @param {string} request.ancestorName
 *   Return activities for this Drive folder and all children and descendants.
 *   The format is `items/ITEM_ID`.
 * @param {google.apps.drive.activity.v2.ConsolidationStrategy} request.consolidationStrategy
 ...
 *   The first element of the array is Array of [DriveActivity]{@link google.apps.drive.activity.v2.DriveActivity}.
 *   The client library will perform auto-pagination by default: it will call the API as many
 *   times as needed and will merge results from all the pages into this array.
 *   Note that it can affect your quota.
 *   We recommend using `queryDriveActivityAsync()`
 ...
 *   Call options. See {@link https://googleapis.dev/nodejs/google-gax/latest/interfaces/CallOptions.html|CallOptions} for more details.
 * @returns {Stream}
 *   An object stream which emits an object representing [DriveActivity]{@link google.apps.drive.activity.v2.DriveActivity} on 'data' event.
 *   The client library will perform auto-pagination by default: it will call the API as many
...
/**
 * Equivalent to `queryDriveActivity`, but returns an iterable object.
 *
 ...
 *   [DriveActivity]{@link google.apps.drive.activity.v2.DriveActivity}. The API will be called under the hood as needed, once per the page,
 ...
 */
  queryDriveActivityAsync(
    ...
    return this.descriptors.page.queryDriveActivity.asyncIterate(
      ...
      callSettings
    ) as AsyncIterable<protos.google.apps.drive.activity.v2.IDriveActivity>;
  }
  ...
  close(): Promise<void> {
    this.initialize();
    if (!this._terminated) {
      return this.driveActivityServiceStub!.then(stub => {
