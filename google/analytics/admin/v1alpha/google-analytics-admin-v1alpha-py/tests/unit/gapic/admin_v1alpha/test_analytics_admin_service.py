...
def test_search_change_history_events(transport: str = 'grpc', request_type=analytics_admin.SearchChangeHistoryEventsRequest):
    client = AnalyticsAdminServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
@pytest.mark.asyncio
async def test_search_change_history_events_async(transport: str = 'grpc_asyncio', request_type=analytics_admin.SearchChangeHistoryEventsRequest):
    client = AnalyticsAdminServiceAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            ...
            '__call__') as call:
        # Designate an appropriate return value for the call.
        call.return_value =grpc_helpers_async.FakeUnaryUnaryCall(analytics_admin.SearchChangeHistoryEventsResponse(
            next_page_token='next_page_token_value',
        ))
...
def test_search_change_history_events_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in pager]
        assert len(results) == 6
        assert all(isinstance(i, resources.ChangeHistoryEvent)
                   for i in results)
...
async def test_search_change_history_events_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_change_history_events(request={})).pages:
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_analytics_admin_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.analytics.admin_v1alpha.services.analytics_admin_service.transports.AnalyticsAdminServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_analytics_admin_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.analytics.admin_v1alpha.services.analytics_admin_service.transports.AnalyticsAdminServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.AnalyticsAdminServiceTransport()
        adc.assert_called_once()
