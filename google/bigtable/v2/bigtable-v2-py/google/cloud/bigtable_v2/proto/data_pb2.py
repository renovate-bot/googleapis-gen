...
# source: google/cloud/bigtable_v2/proto/data.proto
...
Row = _reflection.GeneratedProtocolMessageType('Row', (_message.Message,), {
  ...
  Attributes:
      key:
          ...
          This is the same key that’s used to identify the row in, for
          example, a MutateRowRequest. May contain any non-empty byte
          string up to 4KiB in length.
      families:
          May be empty, but only if the entire row is empty. The mutual
          ordering of column families is not specified.
  """,
...
Family = _reflection.GeneratedProtocolMessageType('Family', (_message.Message,), {
  ...
  Attributes:
      name:
          ...
          ``[-_.a-zA-Z0-9]+``, except that AggregatingRowProcessors may
          produce cells in a sentinel family with an empty name. Must be
          no greater than 64 characters in length.
      columns:
...
Column = _reflection.GeneratedProtocolMessageType('Column', (_message.Message,), {
  ...
  Attributes:
      qualifier:
          ...
          This is the same key that’s used to identify the column in,
          for example, a RowFilter which sets its
          ``column_qualifier_regex_filter`` field. May contain any byte
          string, including the empty string, up to 16kiB in length.
      cells:
...
Cell = _reflection.GeneratedProtocolMessageType('Cell', (_message.Message,), {
  ...
  Attributes:
      timestamp_micros:
          ...
          of 1000.
      value:
          The value stored in the cell. May contain any byte string,
          including the empty string, up to 100MiB in length.
      labels:
...
RowRange = _reflection.GeneratedProtocolMessageType('RowRange', (_message.Message,), {
  ...
  Attributes:
      start_key:
          ...
          set, interpreted as the empty string, inclusive.
      start_key_closed:
          Used when giving an inclusive lower bound for the range.
      start_key_open:
          Used when giving an exclusive lower bound for the range.
      end_key:
          The row key at which to end the range. If neither field is
          set, interpreted as the infinite row key, exclusive.
      end_key_open:
...
ColumnRange = _reflection.GeneratedProtocolMessageType('ColumnRange', (_message.Message,), {
  ...
  Attributes:
      ...
      start_qualifier:
          The column qualifier at which to start the range (within
          ``column_family``). If neither field is set, interpreted as
          the empty string, inclusive.
      start_qualifier_closed:
          Used when giving an inclusive lower bound for the range.
      start_qualifier_open:
          Used when giving an exclusive lower bound for the range.
      end_qualifier:
          The column qualifier at which to end the range (within
          ``column_family``). If neither field is set, interpreted as
          the infinite string, exclusive.
...
ValueRange = _reflection.GeneratedProtocolMessageType('ValueRange', (_message.Message,), {
  ...
  Attributes:
      ...
      start_value_open:
          Used when giving an exclusive lower bound for the range.
      end_value:
          The value at which to end the range. If neither field is set,
          interpreted as the infinite string, exclusive.
      end_value_closed:
          Used when giving an inclusive upper bound for the range.
      end_value_open:
          Used when giving an exclusive upper bound for the range.
...
RowFilter = _reflection.GeneratedProtocolMessageType('RowFilter', (_message.Message,), {
  ...
  'Chain' : _reflection.GeneratedProtocolMessageType('Chain', (_message.Message,), {
    ...
    Attributes:
        filters:
            ...
            input row: in row -> f(0) -> intermediate row -> f(1) -> … ->
            f(N) -> out row The full chain is executed atomically.
    """,
  ...
  'Interleave' : _reflection.GeneratedProtocolMessageType('Interleave', (_message.Message,), {
    ...
    Attributes:
        filters:
            ...
            and the results are pooled, sorted, and combined into a single
            output row. If multiple cells are produced with the same
            column and timestamp, they will all appear in the output row
            in an unspecified mutual order. Consider the following
            ...
            f(0)                      f(1)                      f(2)
            |                         |                         |    1:
            foo,bar,10,x             foo,bar,10,z              far,bar,7,a
            2: foo,blah,11,z            far,blah,5,x
            far,blah,5,x           |                         |
            |
            -----------------------------------------------------
            |    1:                      foo,bar,10,z   // could have
            switched with #2    2:                      foo,bar,10,x   //
            could have switched with #1    3:
            foo,blah,11,z    4:                      far,bar,7,a    5:
            far,blah,5,x   // identical to #6    6:
            far,blah,5,x   // identical to #5  All interleaved filters are
            executed atomically.
    """,
  ...
  'Condition' : _reflection.GeneratedProtocolMessageType('Condition', (_message.Message,), {
    ...
    Attributes:
        predicate_filter:
            ...
            ``true_filter`` will be evaluated on the input row. Otherwise,
            ``false_filter`` will be evaluated.
        true_filter:
        ...
        false_filter:
            The filter to apply to the input row if ``predicate_filter``
            does not return any results. If not provided, no results will
            be returned in the false case.
    """,
  ...
  Attributes:
      filter:
          ...
          set, this RowFilter returns all cells in the input row.
      chain:
          Applies several RowFilters to the data in sequence,
          progressively narrowing the results.
      interleave:
          Applies several RowFilters to the data in parallel and
          combines the results.
      condition:
          Applies one of two possible RowFilters to the data based on
          the output of a predicate RowFilter.
      sink:
          ADVANCED USE ONLY. Hook for introspection into the RowFilter.
          Outputs all cells directly to the output of the read rather
          than to any parent filter. Consider the following example:  ::
          Chain(      FamilyRegex("A"),      Interleave(        All(),
          Chain(Label("foo"), Sink())      ),      QualifierRegex("B")
          )                         A,A,1,w
          A,B,2,x                        B,B,4,z
          |                    FamilyRegex("A")
          |                        A,A,1,w
          ...
          |            All()                    Label(foo)
          |                          |           A,A,1,w
          A,A,1,w,labels:[foo]           A,B,2,x
          A,B,2,x,labels:[foo]              |                          |
          |                        Sink() --------------+              |
          |                  |              +------------+      x------+
          A,A,1,w,labels:[foo]                           |
          A,B,2,x,labels:[foo]                        A,A,1,w
          |                        A,B,2,x                             |
          |                                |
          QualifierRegex("B")                      |
          |                                |
          ...
          A,B,2,x               // could be switched  Despite being
          excluded by the qualifier filter, a copy of every cell that
          reaches the sink is present in the final result.  As with an
          ...
          “A:B” and timestamp 2, because one copy passed through the all
          filter while the other was passed through the label and sink.
          Note that one copy has label “foo”, while the other does not.
      ...
      pass_all_filter:
          ...
          completeness.
      block_all_filter:
          Does not match any cells, regardless of input. Useful for
          temporarily disabling just part of a filter.
      row_key_regex_filter:
          Matches only cells from rows whose keys satisfy the given RE2
          regex. In other words, passes through the entire row when the
          key matches, and otherwise produces an empty row. Note that,
          since row keys can contain arbitrary bytes, the ``\C`` escape
          sequence must be used if a true wildcard is desired. The ``.``
          character will not match the new line character ``\n``, which
          may be present in a binary key.
      row_sample_filter:
          Matches all cells from a row with probability p, and matches
          no cells from the row with probability 1-p.
      family_name_regex_filter:
          ...
          contain the ``:`` character, even if it is not being used as a
          literal. Note that, since column families cannot contain the
      ...
      column_qualifier_regex_filter:
          ...
          in a binary qualifier.
      column_range_filter:
          Matches only cells from columns within the given range.
      timestamp_range_filter:
          Matches only cells with timestamps within the given range.
      value_regex_filter:
          Matches only cells with values that satisfy the given regular
          expression. Note that, since cell values can contain arbitrary
          bytes, the ``\C`` escape sequence must be used if a true
          wildcard is desired. The ``.`` character will not match the
          new line character ``\n``, which may be present in a binary
          value.
      value_range_filter:
          Matches only cells with values that fall within the given
          range.
      cells_per_row_offset_filter:
          Skips the first N cells of each row, matching all subsequent
          cells. If duplicate cells are present, as is possible when
          using an Interleave, each copy of the cell is counted
          separately.
      cells_per_row_limit_filter:
          Matches only the first N cells of each row. If duplicate cells
          are present, as is possible when using an Interleave, each
          copy of the cell is counted separately.
      cells_per_column_limit_filter:
          Matches only the most recent N cells within each column. For
          example, if N=2, this filter would match column ``foo:bar`` at
          timestamps 10 and 9, skip all earlier cells in ``foo:bar``,
          and then begin matching again in column ``foo:bar2``. If
          duplicate cells are present, as is possible when using an
          Interleave, each copy of the cell is counted separately.
      ...
      apply_label_transformer:
          ...
          from which part of the filter.  Values must be at most 15
          characters in length, and match the RE2 pattern
          ``[a-z0-9\\-]+``  Due to a technical limitation, it is not
          currently possible to apply multiple labels to a cell. As a
          result, a Chain may have no more than one sub-filter which
          contains a ``apply_label_transformer``. It is okay for an
          Interleave to contain multiple ``apply_label_transformers``,
          as they will be applied to separate copies of the input. This
...
Mutation = _reflection.GeneratedProtocolMessageType('Mutation', (_message.Message,), {
  ...
  'SetCell' : _reflection.GeneratedProtocolMessageType('SetCell', (_message.Message,), {
    ...
    Attributes:
        ...
        timestamp_micros:
            The timestamp of the cell into which new data should be
            written. Use -1 for current Bigtable server time. Otherwise,
  ...
  'DeleteFromColumn' : _reflection.GeneratedProtocolMessageType('DeleteFromColumn', (_message.Message,), {
    ...
    Attributes:
        family_name:
            ...
            Must match ``[-_.a-zA-Z0-9]+``
        column_qualifier:
            The qualifier of the column from which cells should be
            deleted. Can be any byte string, including the empty string.
        time_range:
...
ReadModifyWriteRule = _reflection.GeneratedProtocolMessageType('ReadModifyWriteRule', (_message.Message,), {
  ...
  Attributes:
      ...
      column_qualifier:
          The qualifier of the column to which the read/modify/write
          should be applied. Can be any byte string, including the empty
          string.
      rule:
          The rule used to determine the column’s new latest value from
          its current latest value.
      append_value:
      ...
      increment_amount:
          ...
          must contain an 8-byte value (interpreted as a 64-bit big-
          endian signed integer), or the entire request will fail.
  """,
