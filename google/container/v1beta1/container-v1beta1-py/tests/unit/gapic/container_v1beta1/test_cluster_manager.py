...
from google.container_v1beta1.services.cluster_manager.transports.base import _API_CORE_VERSION
from google.container_v1beta1.services.cluster_manager.transports.base import _GOOGLE_AUTH_VERSION
...
def test_list_locations(transport: str = 'grpc', request_type=cluster_service.ListLocationsRequest):
    client = ClusterManagerClient(
        credentials=credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_cluster_manager_base_transport_with_credentials_file():
    ...
    with mock.patch.object(auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.container_v1beta1.services.cluster_manager.transports.ClusterManagerTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_cluster_manager_base_transport_with_adc():
    ...
    with mock.patch.object(auth, 'default', autospec=True) as adc, mock.patch('google.container_v1beta1.services.cluster_manager.transports.ClusterManagerTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transport = transports.ClusterManagerTransport()
        adc.assert_called_once()
