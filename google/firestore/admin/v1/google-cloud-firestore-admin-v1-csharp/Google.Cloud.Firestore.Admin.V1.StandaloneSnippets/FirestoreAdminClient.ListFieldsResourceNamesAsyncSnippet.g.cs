...
namespace Google.Cloud.Firestore.Admin.V1.Snippets
{
    ...
    public sealed partial class GeneratedFirestoreAdminClientStandaloneSnippets
    {
        ...
        public async Task ListFieldsResourceNamesAsync()
        {
            ...
            // Make the request
            PagedAsyncEnumerable<ListFieldsResponse, Field> response = firestoreAdminClient.ListFieldsAsync(parent);

            // Iterate over all response items, lazily performing RPCs as required
            await response.ForEachAsync((Field item) =>
            {
            ...
            int pageSize = 10;
            Page<Field> singlePage = await response.ReadPageAsync(pageSize);
            // Do something with the page of items
            Console.WriteLine($"A page of {pageSize} results (unless it's the final page):");
            foreach (Field item in singlePage)
            {
                // Do something with each item
                Console.WriteLine(item);
            }
